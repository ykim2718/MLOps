...
4) 다운로드 / 삭제 / 원본 객체 보기 (모두 boto3, mc 불필요):
       ...
       python catalog.py remove   epc/v1             # 그 key 의 객체 + catalog 행 영구 삭제
...
def _locate(path: str, spec_dir: Optional[Path] = None) -> str:
    ...
    if len(hits) > 1:
        raise ValueError(
            ...
            + "\n  ".join(hits)
            + "\nuse an absolute --path, or keep it under only one location.")
    return hits[0]
...
def download(minio_key: str, dest: Optional[str] = None,
    ...
    row = get(minio_key)
    if not row:
        raise LookupError(f"not in catalog: '{minio_key}'")
...
def remove(minio_key: str, *, yes: bool = False, member: Optional[str] = None) -> None:
    ...
    row = get(minio_key)                           # derive the real bucket from a catalog row if any
    bucket = urlparse(row["minio_path"]).netloc if row else "datasets"
    prefix = f"{minio_key}/"
    shown = f"s3://{bucket}/{prefix}"
...
def get(minio_key: str) -> Optional[dict]:
    """minio_key 한 행을 반환. 없으면 None."""
    with _conn() as c, c.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("SELECT * FROM datasets WHERE minio_key=%s", [minio_key])
        return cur.fetchone()
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    sp = sub.add_parser("list", help="[PostgreSQL] list registered datasets (minio_key summary)")
    ...
    sp.add_argument("minio_key", type=str)
    sp.add_argument("dest", type=str, nargs="?", default=None, help="default: ./<minio_key>")
    _add_block(sp)
    ...
    sp.add_argument("minio_key", type=str, nargs="?", default=None,
                    help="limit to a minio_key prefix (default: all)")
    _add_block(sp)
