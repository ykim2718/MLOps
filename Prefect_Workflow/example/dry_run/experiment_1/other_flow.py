"""example/dry_run/experiment_1/other_flow.py — git-delivered ML payload, Prefect dry run.

Same flow as example/dry_run/my_flow.py — this copy exists only to show that the payload
does NOT have to be named my_flow.py, nor live at the repo root. pipeline.py runs whatever the
...
Prefect run log + MLflow, so `python other_flow.py --data-folder <dir>` walks the
whole prepare -> featurize -> train -> validate / test path end to end.

Run by pipeline.py (orchestrator) when the run's payload = experiment_1/other_flow.py:
    python other_flow.py --submitter <m> --data-folder <dir>
...
@flow(name="experimental_flow", flow_run_name="{submitter}", log_prints=True)
def experimental_flow(*, submitter: str = "local", data_folder: str = "./data") -> State:
    log = get_run_logger()
