...
| Command | Target | Working<br>Tree | State | Commit<br>History |
|---------|--------|--------------|-------|---------|
| `restore <file>` | Working Tree 의 파일 | 복원 | modified → committed (변경 버림) | 없음 |
| `restore --staged <file>` | Stage (언스테이징) | 유지 | staged → modified | 없음 |
| `checkout <commit>` | HEAD 위치 (작업 트리째) | 교체 | committed 기준으로 맞춤 | HEAD 이동 (이력 보존) |
| `reset --soft` | 커밋 위치 | 유지 | committed → staged | **이력 이동** |
| `reset --mixed` | 커밋·Stage | 유지 | committed → modified | **이력 이동** |
| `reset --hard` | 커밋·Stage·Working Tree | 삭제 | committed → clean (버림) | **이력 이동** |
| `revert <commit>` | 특정 커밋의 효과 | 갱신 | 새 committed 추가 | 새 커밋으로 상쇄 |
