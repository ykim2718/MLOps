...
| Command | Target | Working Tree | State | History | Safety |
|---------|--------|--------------|-------|---------|--------|
| `restore <file>` | Working Tree 의 파일 | 복원 | modified → committed (변경 버림) | 없음 | 변경분만 사라짐 |
| `restore --staged <file>` | Stage (언스테이징) | 유지 | staged → modified | 없음 | 안전 |
| `checkout <commit>` | HEAD 위치 (작업 트리째) | 교체 | committed 기준으로 맞춤 | HEAD 이동 (이력 보존) | detached 주의 |
| `reset --soft` | 커밋 위치 | 유지 | committed → staged | **이력 이동** | 비교적 안전 |
| `reset --mixed` | 커밋·Stage | 유지 | committed → modified | **이력 이동** | 기본값 |
| `reset --hard` | 커밋·Stage·Working Tree | 삭제 | committed → clean (버림) | **이력 이동** | 위험 |
| `revert <commit>` | 특정 커밋의 효과 | 갱신 | 새 committed 추가 | 새 커밋으로 상쇄 | 안전 (공유 가지 권장) |
...
`checkout` 이 local state 와 working tree 를 바꾸는 **undo (되돌리기)** 라면, `worktree` 와 `archive` 는 working tree 만 바꾸는 **추출 (재현)** 입니다.
...
- **HEAD** — 현재 branch pointer. 보통 현재 가지의 맨 끝 커밋을 가리키며, 가지가 아닌 특정 커밋을 직접 가리키면 detached 상태입니다.
