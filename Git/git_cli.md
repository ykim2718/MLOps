...
원격에 올라간 nested 내용을 내 쪽으로 받아 펼칩니다. submodule·subtree 는 위 Creation 의 명령으로 받고 (submodule 은 `clone --recurse-submodules` · `submodule update --init`, subtree 는 `subtree pull`), 특정 버전만 가볍게 떨어뜨릴 때는 worktree·archive 를 씁니다.

#### Worktree — pin a version

다른 저장소를 한 번 받아 두고, 원하는 커밋을 하위 폴더로 펼칩니다. `.gitmodules` 없이 특정 버전을 고정해 쓸 때 좋습니다.
...
`.git` 없이 특정 커밋의 파일만 하위 폴더에 풉니다. 받기만 하고 추적은 하지 않을 때 씁니다.

```bash
git -C <dep> archive <commit> | tar -x -C <path>        # <commit> 시점 파일만 <path> 에 푼다(.git 없음).
git -C <dep> archive --format=zip -o snap.zip <commit>  # zip 으로 묶어 받는다.
```

> worktree·archive 로 받은 폴더는 다른 저장소의 사본이라, 메인 저장소에서는 보통 `.gitignore` 로 빼 두어 메인 이력에 담지 않습니다.
