...
한 저장소에 작업 트리를 여러 개 두어, 가지마다 별도 폴더에서 동시에 작업합니다. 가지를 옮길 때 stash 로 치우지 않아도 되고, 각 폴더가 같은 `.git` 이력을 공유합니다.

```bash
git worktree list                          # 지금 딸려 있는 작업 트리를 모두 본다(경로·가지·커밋).
...
> 같은 가지를 두 작업 트리에서 동시에 펼칠 수는 없습니다. 임시로 한 커밋만 들여다보려면 `--detach` 가 깔끔합니다.

## 10. Archive

가지나 커밋의 한 시점을 `.git` 이력 없이 압축 파일로 내보냅니다. 소스만 묶어 배포하거나 넘길 때 씁니다.
...
> `git archive` 는 추적 중인 파일만 담습니다. `.gitignore` 로 제외된 파일이나 `.git` 폴더는 들어가지 않으므로, 배포용 스냅샷에 알맞습니다.

## 11. Windows — Long Paths
