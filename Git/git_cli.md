...
```text
 Working Tree      Stage (Index)        Local Repo          Remote Repo
 (작업 중 파일)  →   (담아 둔 변경)   →   (커밋된 이력)   ⇄   (원격 저장소)
...
## 1. Repository Setup — init · remote
...
## 4. Branch & Switch — branch · switch · checkout
...
## 7. Undo — restore · checkout · reset · revert

되돌리기는 **"어느 영역을 되돌리느냐"** 로 갈립니다.

| Command | 되돌리는 대상 | 이력 변화 | 안전도 |
|---------|---------------|-----------|--------|
