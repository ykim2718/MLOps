...
- **필요**: 코드가 실행되는 그 컴퓨터(예: user의 로컬 머신)에서 자격증명을 쓸 수 있게 해줘야 함 — ① 그 컴퓨터에 **환경변수**로 등록하거나 ② **Prefect Secret**(Prefect 서버에 저장해 두고 코드가 이름으로 불러옴). 셋업 방법은 아래 [Credential Setup](#credential-setup) 참고.
- **불필요**: **user별 별도 자격증명이 필요 없음** — 잡을 서버/워커가 대신 실행하므로, user는 서버 주소(`PREFECT_API_URL`)만 설정하면 됨.

```powershell
prefect config set PREFECT_API_URL="http://<서버>:4200/api"   # 서버 주소(최초 1회)
```
...
표의 **Credentials = 필요** 인 경우, user 쪽 코드가 MinIO·카탈로그 자격증명을 공급해야 합니다. `catalog.resolve()` 가 **환경변수 → Prefect Secret → 기본값** 순으로 찾으므로, 아래 둘 중 하나로 셋업합니다.

**방법 1 — 환경변수**
실행하는 컴퓨터에 자격증명을 환경변수(`MINIO_ACCESS_KEY` / `MINIO_SECRET_KEY` / `MINIO_ENDPOINT` / `POSTGRESQL_CATALOG_DSN`)로 설정해 두면 코드가 `os.environ` 에서 읽습니다. (`docker-compose.env` 값을 한 번에 환경변수로 올리는 헬퍼는 [Appendix E](#appendix-e-set_environmentps1) 참고.)

**방법 2 — Prefect Secret**
자격증명을 Prefect 서버에 Secret/Variable 로 저장해 두고 코드가 *이름으로* 불러옵니다. admin이 한 번 등록합니다([`register_blocks.py`](register_blocks.py)):
...
등록되는 것: Secret `minio-access-key` / `minio-secret-key` / `catalog-dsn`, Variable `minio_endpoint`. 이후 코드는 그대로 동작합니다(환경변수가 없으면 Secret 사용):

```python
import catalog
ep = catalog.resolve("MINIO_ENDPOINT", "minio_endpoint", default="http://localhost:9000", secret=False)
ak = catalog.resolve("MINIO_ACCESS_KEY", "minio-access-key")   # env 있으면 env, 없으면 Secret
```
...
`docker-compose.env`는 **서버 admin 전용**입니다. 자격증명·엔드포인트는 yml 에 평문으로 두지 않고 **`docker-compose.env` 한 파일에 모읍니다.** 컨테이너는 각 서비스가 `env_file`로 읽고, **호스트 파이썬(`catalog.py` 등)은 이 값들을 환경변수로 올린 뒤 `os.environ`에서 읽습니다**([Appendix E](#appendix-e-set_environmentps1) 참고). 실제 값이 담긴 `docker-compose.env`는 **`.gitignore`로 제외**하고, 비밀값을 비운 아래 **`docker-compose.env_example`만 커밋**합니다. 새 환경에서는 이 예시를 복사해 모든 `CHANGE_ME`를 채우면 됩니다.
...
## Appendix E. set_environment.ps1

`docker-compose.env` 의 `KEY=VALUE` 들을 **호스트 환경변수로 올리는** 헬퍼입니다(`docker-compose.env` 와 같은 폴더에 둠). 한 번 실행하면 이후 호스트 파이썬(`catalog.py` 등)이 `os.environ` 에서 읽습니다 — [Credential Setup](#credential-setup) 방법 1에서 사용.

```powershell
.\set_environment.ps1            # 현재 세션 + 사용자(User) 영구 환경변수에 설정(기본)
.\set_environment.ps1 -Session   # 현재 세션에만 설정
```
...
foreach ($line in Get-Content -LiteralPath $EnvFile) {
    ...
    if (-not $Session) { [Environment]::SetEnvironmentVariable($k, $v, 'User') }   # User 영구
    Write-Host "set $k"                                              # 값은 출력하지 않음(시크릿 보호)
}
```
