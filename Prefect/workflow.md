...
| **Optuna**     | tuner               | 하이퍼파라미터 튜닝(trial 탐색).<br>목적: 재현성(하이퍼파라미터 고정/탐색)                                                                                                          | study storage는<br>`postgres`(`optuna` DB)                    |                                                                     | [5](#5-optuna)     |
...
**MinIO**는 이 스택의 **오브젝트 스토리지(S3 호환)** 로, 데이터셋·모델·MLflow artifact 등 **실제 대용량 데이터**를 보관합니다(메타데이터는 PostgreSQL, 실제 파일은 MinIO).

- 버킷: `datasets` / `models` / `mlflow` (콘솔 http://localhost:9001, S3 API :9000)
- 설치 · 버킷 생성 · 버저닝 · boto3 접근 · 모델 저장 위치 등 **상세는 → [minio.md](minio.md)**
...
> 요약: 개별 실험이면 best 5는 **MinIO로 충분**합니다. 배포·서빙까지 가면 그 모델을 **MLflow Model Registry**에 등록하면 됩니다.
...
> 어느 쪽이든 Optuna가 DB에 넣는 것은 **trial 메타데이터(파라미터·점수)뿐**이고, 모델 가중치 같은 실제 산출물은 DB가 아니라 MinIO에 저장합니다([4. MLflow — Where to Store the Best Models](#where-to-store-the-best-models-weights) 참고).
...
스택 전체(postgres · minio · createbuckets · mlflow · server · worker)를 `docker compose up -d` 로 한 번에 띄웁니다. **실행 방법 · 서비스 구성 · `docker-compose.yml` 설명 · 운영 모델 등 상세는 → [prefect.md](prefect.md)** 참고.
