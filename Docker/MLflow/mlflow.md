...
# (1) search — 지표 기준으로 상위 run 을 고른다 ("best" 는 log_metric 값 기준).
...
# (2) load — download 없이 메모리로 바로 로드해 추론.
model = mlflow.pyfunc.load_model(f"runs:/{run_id}/model")
pred = model.predict(X)

# (3) download — 로컬 폴더로 산출물을 받는다.
...
# (1) search — 등록된 이름·버전·Stage·원본 run·실제 위치를 조회.
