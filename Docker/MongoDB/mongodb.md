...
services:
  mongo:
    ...
    logging:
      driver: json-file             # default driver. stores stdout logs as JSON files.
      options:
        max-size: "10m"             # rotate when a single file exceeds 10MB.
        max-file: "10"              # keep up to 10 files, deleting the oldest first.
...
### Data Location
  ...
  services:
    mongo:
      volumes:
        - ./mongo-data:/data/db    # store in a mongo-data folder next to the compose file (directly accessible)
...
### Python (`pymongo`)
  ...
  # URI example: mongodb://<user>:<password>@<host>:27017/?authSource=admin
  client = MongoClient(os.environ["MONGODB_URI"])
  db = client["yControl"]                                            # select the DB (created on first write if absent)
  db["schedule_board"].insert_one({"task": "demo", "done": False})  # the collection is auto-created too
...
### CLI (`mongosh`)
  ...
  # use the container's mongosh directly (when not installed on the host)
...
```javascript
// (1) create a user — assign roles per DB. user info is stored in the admin DB.
use admin
db.createUser({
  user: "analyst",
  pwd:  "CHANGE_ME",
  roles: [
    { role: "readWrite", db: "yControl" },   // yControl: read/write
    { role: "read",      db: "yImprove" }     // yImprove: read-only
  ]
})

// (2) check · modify · delete
db.getUser("analyst")                                                  // check permissions
db.grantRolesToUser("analyst", [{ role: "read", db: "yReport" }])      // add a role
db.revokeRolesFromUser("analyst", [{ role: "read", db: "yImprove" }])  // revoke a role
db.dropUser("analyst")                                                 // delete the user
```
...
### `docker compose exec` 방식
  ...
  ```bash
  # (1) container status — STATUS 'Up (healthy)' means the healthcheck (mongosh ping) passed too.
  docker compose -f docker-compose.yml ps

  # (2) ping — '{ ok: 1 }' means it's healthy.
  docker compose -f docker-compose.yml exec mongo \
    mongosh -u "$MONGO_INITDB_ROOT_USERNAME" -p "$MONGO_INITDB_ROOT_PASSWORD" \
    --quiet --eval 'db.adminCommand({ ping: 1 })'

  # (3) auth & DB list — if you log in with the root account and see the admin / config / local system DBs, auth is on.
  #     (yControl / yImprove are not visible until the first write — lazy creation.)
  docker compose -f docker-compose.yml exec mongo \
    mongosh -u "$MONGO_INITDB_ROOT_USERNAME" -p "$MONGO_INITDB_ROOT_PASSWORD" \
    --quiet --eval 'show dbs'

  # (4) logs — check the cause when something goes wrong.
...
### `docker exec` 방식
  ...
  ```bash
  # (1) container status — list all containers and check mongo's STATUS.
  docker ps
  ...
  # (3) auth & DB list — if you see the admin / config / local system DBs, auth is on.
  docker exec mongodb-mongo-1 \
    mongosh -u "$MONGO_INITDB_ROOT_USERNAME" -p "$MONGO_INITDB_ROOT_PASSWORD" \
    --quiet --eval 'show dbs'
  ...
  # (5) interactive shell — enter the mongosh prompt and type commands like 'show dbs' directly.
