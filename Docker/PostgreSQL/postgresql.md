...
Rev. 53 | Created: 2026-06-15 | Updated: 2026-08-14 21:32 CDT
...
services:
  postgres:
    image: postgres:16
    env_file:
      - docker-compose.env_example  # injects POSTGRES_USER / POSTGRES_PASSWORD
    ports:
  ...
  pgadmin:
    ...
    env_file:
      - docker-compose.env_example  # injects PGADMIN_DEFAULT_EMAIL / PGADMIN_DEFAULT_PASSWORD
    ports:
...
- `name: postgresql` 은 프로젝트명을 파일에 고정합니다. 이 값이 컨테이너·볼륨 이름의 앞가지가 되므로, `-p` 를 붙이지 않아도 (혹은 다른 폴더에서 띄워도) 항상 같은 프로젝트·같은 볼륨에 붙어 등록한 데이터가 어긋나지 않습니다.
- `image: postgres:16` 은 공식 PostgreSQL 16 이미지를 사용한다는 뜻입니다.
- `env_file` 은 계정·비밀번호를 yml 에 평문으로 두지 않고 `docker-compose.env_example` 에서 읽어 주입합니다.
...
`docker compose up` 으로 뜬 컨테이너 이름은 `<Project Name>-<Service Name>-<Replica Number>` 형식 (예: `postgresql-postgres-1`) 이며, Replica Number 는 보통 `1` 이지만 `--scale <service>=3` 처럼 늘리면 `-2`·`-3` 이 추가됩니다.
...
### Credentials
  ...
  접속 계정은 `docker-compose.env_example` 한 곳에 모으고, 컨테이너는 `env_file` 로 읽으며 호스트 파이썬은 같은 값을 환경변수로 올려 `os.environ` 에서 읽습니다. 이 학습 스택은 실제 비밀값을 쓰지 않으므로 placeholder 가 담긴 이 파일을 그대로 커밋해 씁니다. 실제 비밀값이 필요한 환경이라면 이 파일에 적지 말고 git 에서 제외한 별도 파일에 담아 `env_file` 이 그쪽을 읽게 합니다.

  ```dotenv
  # docker-compose.env_example  (placeholders for the study stack — PGADMIN_DEFAULT_EMAIL must be a valid email format)
  ...
  - `PGADMIN_DEFAULT_EMAIL` 만 형식상 유효한 이메일 (예: `admin@example.com`) 이어야 pgAdmin 이 기동되고, 나머지는 placeholder 그대로 둡니다. 실서비스라면 강한 값으로 바꾸되 git 에 올라가지 않는 파일에 둡니다.
