...
- **버전 고정** — 베이스 이미지를 `python:3.11.15` 로 못 박아 인터프리터 버전을 통일합니다.
...
```powershell
# (최초 1회) 예시 파일을 복사해 연결 정보를 채운다. docker-compose.env 는 git 에 커밋하지 않는다.
Copy-Item docker-compose.env_example docker-compose.env

# 공유 네트워크 mlops 를 만들고 (이미 있으면 그대로 둔다) 이미지를 빌드해 컨테이너를 띄운다.
...
```text
numpy==1.26.4                  # 수치 배열 (전체 의존성의 기준 버전)
pandas==2.0.3                  # 표 형식 데이터 처리
scikit-learn==1.4.2            # 전통적 머신러닝 알고리즘
TA-Lib==0.4.29                 # 기술적 분석 지표 (C 라이브러리 필요)
```
...
services:
  python:
    ...
    env_file:
      - docker-compose.env          # 연결 정보를 주입한다.
    volumes:
    ...
    restart: "no"                   # YAML 이 no 를 boolean 으로 해석하므로 따옴표로 감싼다.
...
기본 진입점 (`run.py`) 이 아닌 임의의 스크립트를 실행하려면, compose 의 기본 명령을 덮어쓰면 됩니다. `docker compose run` 은 `docker-compose.yml` 의 볼륨·네트워크·환경변수를 그대로 적용한 채 일회성으로 컨테이너를 띄웁니다.

```bash
# app/ 가 /app 에 마운트된 채, 컨테이너 파이썬으로 example.py 를 실행한다.
...
> `docker compose up` 은 `docker-compose.yml` 의 기본 명령 (`CMD ["python", "run.py"]`) 을 실행하고, `docker compose run python python example/example.py` 는 그 명령을 `python example/example.py` 로 덮어씁니다. 즉 같은 이미지·같은 환경에서 **실행할 파일만 바꾸는 것**입니다.

### 떠 있는 컨테이너 안에서 실행 — `docker compose exec`

데이터를 바꿔 가며 여러 번 실행할 때는, 컨테이너를 살려 둔 채 그 안으로 들어가는 편이 빠릅니다. 단발성 `run.py` 대신 컨테이너를 계속 띄워 두려면 기동 명령을 대기 상태로 바꿉니다.

```bash
# (1) 컨테이너를 대기 상태로 띄운다 (기본 CMD 를 sleep 으로 덮어쓴다).
docker compose run -d --name py-dev python sleep infinity

# (2) 컨테이너 안에서 실행하거나 셸로 진입한다.
docker compose exec python python example/example.py
docker compose exec python bash          # 대화형 셸
...
# 모든 CHANGE_ME 를 실제 값으로 채운다. 이 파일은 비밀값을 담으므로 .gitignore 로 git 추적에서 제외한다.
...
# (2) 대상 폴더의 코드를 분석해 그 폴더에 requirements.txt 를 생성한다.
pipreqs ./app --encoding=utf-8

# (3) 이미 파일이 있어 덮어쓰려면 --force 를 붙인다.
