...
- **Secret 블록**은 민감한 값을 Prefect server (backend DB) 에 **이름 붙여 저장**하고 코드가 이름으로 꺼내 쓰는 Prefect 내장 기능이며, UI·로그에서는 값이 가려진다.
...
스크립트를 1회 실행해 Secret을 server (또는 Prefect Cloud) 에 저장한다. `overwrite=True` 면 같은 이름이 있을 때 덮어쓴다.

**문자열 하나를 저장한다:**
```python
from prefect.blocks.system import Secret

block: Secret = Secret(value="my-secret-key")   # Secret block holding the secret string
block.save(name="api-key", overwrite=True)      # save to the server under the name 'api-key'
```
...
def my_pipeline() -> dict:
    creds: dict = Secret.load("my-confidentials").get()   # → dict (decrypted actual values)
...
```python
# use — pick the block for the running user
import os
from prefect.blocks.system import Secret
...
> ⚠️ **self-hosted Prefect (OSS) server 는 기본 API 인증이 없어**, API에 접근 가능한 사람은 모든 Secret을 읽을 수 있다. 이 방식은 **이름만 분리될 뿐 접근 차단은 아니므로**, 진짜 격리가 필요하면 각 머신에 **환경변수**로 분리하거나 Prefect Cloud (워크스페이스/RBAC), 또는 Vault 같은 시크릿 매니저를 쓴다.

---

## 6. Notes

- **저장 위치**: Secret 값은 Prefect server 의 backend DB (Prefect 메타데이터 DB) 안 block 테이블에 저장된다.
- **Variable 과의 차이**: **Variable** 은 Prefect 가 server 에 이름-값 쌍을 저장해 두고 flow 코드나 deployment 에서 이름으로 불러 쓰는 또 다른 내장 저장소다 (`prefect.variables`). Variable 도 Secret 처럼 server 에 저장되지만, 값을 가리지 않고 **평문 그대로** 보관한다. 따라서 비밀번호·키 같은 민감한 값은 Secret 에 두고, endpoint·플래그처럼 민감하지 않은 설정값은 Variable 에 둔다.
