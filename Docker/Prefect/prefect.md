...
Prefect server (`prefect_server`) 는 job 요청을 중앙에서 수집·스케줄링하는 **단일 진입점 (Single Point of Entry)** 입니다. 다만 **`prefect_server` 는 코드를 실행하지 않습니다** — 코드는 항상 실행기 (`python` 프로세스 또는 `prefect_worker`) 가 떠 있는 컴퓨터에서 실행됩니다.
...
worker 1개가 동시에 실행하는 job 수는 `--limit` 값 (현재 8) 입니다. worker 는 work pool 에서 job 을 가져와 실행하는데, `prefect worker start --pool default --limit 8` 의 `--limit` 이 그 worker 의 **동시 실행 상한** 입니다. 9번째 job 은 앞 job 하나가 끝나 slot 이 빌 때까지 대기열에서 기다립니다. 처리량을 늘리는 방법은 두 가지입니다 — `--limit` 을 키우거나 (`--limit N`), worker 수를 늘립니다 (`docker compose -p <Project Name> -f docker-compose.worker.yml up -d --scale prefect_worker=3`). 이때 **전체 동시 실행 수 ≈ worker 수 × `--limit`** 입니다. 다만 과도하게 키우지 말고 Worker Node 의 **CPU/GPU/메모리 한도** 안에서 정해야 하며 (자원 경합 시 오히려 느려집니다), GPU 학습처럼 1job 이 자원을 많이 쓰면 `--limit` 을 1~2 로 낮추는 편이 안전합니다.
...
- **구조**: Prefect server 와 **실제 인프라** (코드가 실제로 실행되는 컴퓨터·컨테이너 — 예: GPU 머신, worker 가 떠 있는 노드) 사이에 중간 매개체인 Work Pool (큐) 과 Worker (에이전트) 를 두는 분산 구조입니다.
...
- **push vs pull** — Serve mode 는 실행할 프로세스 (인프라) 를 미리 띄워 그 deployment 전용으로 **고정 (static)** 해 두고, 큐에서 작업을 가져오는 단계 없이 그 프로세스가 곧장 실행하는 **push** 모델입니다. Work pool mode 는 worker 가 work pool 큐를 주기적으로 들여다보며 작업을 스스로 가져오고 (**pull**) 실행 환경을 매 run 마다 **동적 (dynamic)** 으로 구성합니다.
...
특정 머신에서만 실행하려면 (예: 내 GPU) 그 머신 **전용 pool** 로 라우팅합니다 (Case E). `default` 에 붙이면 분산되어 고정이 안 됩니다. 전용 pool 생성·worker 붙이기는 [Appendix D](#appendix-d-additional-worker-attachment) 를 참고합니다.
...
> **선택 사항입니다.** Worker Node 하나로 충분하면 건너뛰어도 됩니다. user 가 자기 컴퓨터의 로컬 자원 (특히 GPU) 에서 학습을 실행하거나 처리량을 분산하려는 경우, 다른 머신에서 worker 를 더 띄워 같은 server 의 work pool 에 붙입니다.
...
특정 머신에서만 실행하려면 그 머신 전용 pool 을 만들고 그 머신의 worker 만 그 pool 을 폴링하게 합니다 (§6 Case E). deployment 를 그 pool 로 보내면 항상 그 머신에서 실행됩니다.
...
```powershell
$env:CREATE_POOL="false"; $env:WORK_POOL="pool-1"; docker compose -p <Project Name> -f docker-compose.worker.yml up -d
```
