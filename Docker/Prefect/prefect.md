...
모든 구성요소는 한 호스트에서 공유 네트워크 `mlops` 로 묶입니다. `prefect_server` 와 `prefect_worker` (dispatcher) 가 상시 떠 있고, job 마다 **Pipeline Flow 컨테이너** 가 떴다 사라집니다.
...
| **2. Prefect Worker** | dispatcher / executor | Short Term (주력) 에서는 pool 을 polling 해 job 마다 Pipeline Flow 컨테이너를 띄웁니다. Long Term 에서는 자기 컨테이너에서 flow 를 직접 실행합니다. | 상시 |
| **3. Pipeline Flow** | execution unit | flow (코드) 가 실제로 실행되는 곳입니다. Short Term 은 job 마다 뜨는 전용 ephemeral 컨테이너, Long Term 은 Prefect Worker 컨테이너 안 (별도 없음) 입니다. | Short Term: 일시적 / Long Term: 상시 |
...
- **동시성** — dispatcher 는 job 마다 같은 이미지로 **독립 컨테이너를 동시에** 띄웁니다. 서로 상태를 공유하지 않고 (`git checkout` 도 각자), 끝나면 파괴되어 동시성 관리가 단순합니다.
...
| `minio` | `:9000` (S3 API) · `:9001` (console) | 오브젝트 스토리지. 데이터·모델·아티팩트를 보관합니다. |
...
- **pool 타입** — 실행 환경을 만드는 방식이 갈립니다.
  ...
  - **Short Term Container** (`docker`, 주력) — job 마다 새 컨테이너 (Pipeline Flow) 를 띄워 실행하고 파괴합니다. run 마다 격리되어 다수 팀원·동시 실행에 적합합니다.
...
- **Short Term Container** (`docker` work pool, 주력) — job 마다 Pipeline Flow 컨테이너를 띄웠다 정리합니다.
...
- `volumes` — **데이터 캐시용 공유 볼륨**. 컨테이너는 일시적이라 `data/` 가 매번 사라지므로 볼륨을 마운트해 캐시를 보존합니다. 버전 경로 (`v3_best` 등) 는 불변이라 공유해도 안전합니다. `data/` 는 gitignore 해 `git reset --hard` 가 캐시를 건드리지 않게 합니다.
...
job 마다 뜨는 컨테이너의 python 환경입니다. **팀 공통 소스를 구워 두고** (`.git` 포함), 런타임에 `git fetch && git checkout <commit>` 으로 원하는 커밋으로 전환합니다. 라이브러리·소스가 한 이미지에 고정되어 모두 같은 런타임을 씁니다.
...
- 소스를 구워 두면 `.git` 이 이미 있어 `git checkout` 이 빠릅니다. `git fetch` 는 빌드 이후 새 커밋만 받습니다.
...
- **Pipeline Flow** — dispatcher 가 job 마다 띄우는 일시적 실행 컨테이너입니다. 받은 커밋으로 checkout 한 뒤 코드를 실행하고 끝나면 파괴됩니다.
- **Short Term Container (ephemeral container)** — `docker` work pool 이 job 마다 띄웠다 파괴하는 일시적 컨테이너입니다. 이 문서의 Pipeline Flow 가 여기 해당합니다.
- **Long Term Container** — `process` work pool 에서 여러 job 을 subprocess 로 실행하는 공유 상시 컨테이너 (worker 컨테이너 자체) 입니다.
...
- **ST·LT 동일 이미지** — ST 는 `pipeline-flow` 이미지를 job 마다 띄우고, LT 는 같은 이미지를 worker 로 상시 띄웁니다. 라이브러리·소스가 이미지에 있어 코드는 git checkout 으로 전환할 뿐 별도 전달·설치가 없습니다 (worker 에 라이브러리를 따로 설치하지 않으므로 중복이 없습니다).
