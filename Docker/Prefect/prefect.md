...
3) **PIPELINE FLOW** — job 마다 떴다 사라지는 컨테이너 · 직접 빌드
   ...
   ```powershell
   .\build_flow.ps1 -Image pipeline-flow:latest
...
```powershell
# build_flow.ps1 — (once) build the Pipeline Flow image (orchestrator + team libraries).
param(
    [string]$Image = 'pipeline-flow:latest'   # the flow image tag (runtime version)
)

$ErrorActionPreference = "Stop"

# Build from this folder (Dockerfile + requirements.txt + pipeline.py). The team repo is cloned at runtime per run.
