...
등록된 deployment 를 실제로 돌리는 (trigger) 방법은 여러 가지지만, 결국 모두 **server 의 Prefect API 에 "flow run 생성" 요청을 보내는 것**입니다 — 코드가 아니라 **deployment 이름 + 파라미터 값** 만 보냅니다. 그러면 server 가 run 을 work pool 에 얹고, dispatcher 가 `pipeline_flow` 컨테이너를 띄워 그 안에서 `pipeline(**parameters)` 가 실행됩니다. 그래서 어디서 trigger 하든 실행 위치·격리는 같습니다 ([§4.3](#43-pipelinepy)).
...
### 6.1 Prefect CLI
  ...
  prefect deployment run "pipeline/pipelineflow-high" `
    -p git_repo=https://github.com/team/repo.git -p git_commit=a1b2c3d -p minio_key=SYDNEY/001.parquet -p member=alice
  ```
...
### 6.2 Python SDK
  ...
  flow_run = run_deployment(                                                   # ask the server to create a flow run
      ...
      parameters={"git_repo": "https://github.com/team/repo.git",
                  "git_commit": "a1b2c3d", "minio_key": "SYDNEY/001.parquet", "member": "alice"},
  )
