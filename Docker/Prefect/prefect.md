...
Pipeline Flow 는 dispatcher 가 job 마다 띄우는 per-flow 컨테이너입니다. dispatcher 하나가 동시 job 수만큼 **여러 개 (n 개)** 를 띄우며 (상한 `--limit`, 현재 8), 각 컨테이너는 독립입니다. 세 가지를 다룹니다 — 컨테이너가 쓰는 **이미지** ([§4.1](#41-image)), 그 이미지로 무엇을 실행할지 server 에 등록하는 **deployment** ([§4.2](#42-deployment)), 컨테이너 안에서 generic flow orchestrator 역할을 하는 `pipeline.py` ([§4.3](#43-pipelinepy)). dispatcher 자신은 flow 를 실행하지 않으므로 flow 는 **별도 이미지** 를 쓰며 ([§3.1](#31-image)), 팀 라이브러리는 이 flow 이미지에만 둡니다.
...
### 4.3 pipeline.py
  ...
  `@flow(name="pipeline")` 의 `name` 이 **flow 이름** 이 되고, deployment 는 server·UI 에서 `<flow_name>/<deployment_name>` (예: `pipeline/pipelineflow-high`) 로 표시·참조됩니다 ([Appendix A](#appendix-a-terminology)).
  ...
  def pipeline(git_repo: str, git_commit: str, minio_version: str, member: str = "", payload: str = "train.py"):
  ...
  - **이력 자동 저장** — `@flow` 진입 시 Prefect 가 run 의 상태·로그·파라미터를 자동 기록합니다 (대시보드 Flow Runs). 지표·모델은 팀원 코드가 MLflow 로 로깅하면 함께 남습니다 ([Appendix H](#appendix-h-prefect-task)).
  - **누구의 run 인지** — `member` 가 **flow 파라미터** 라 `@flow(flow_run_name="{member}@{git_commit}")` 로 run 이름에 들어가, UI 의 `pipeline/pipelineflow-high` 아래에서 `alice@a1b2c3d` 처럼 구분됩니다. `pipeline.py` 가 `MEMBER`·`GIT_COMMIT` 을 payload env 로 넘기므로 팀 flow run 도 같은 이름으로 표시할 수 있습니다.
  - **단계 가시성** — 팀 payload 가 단계 (dp·fe·train·test) 를 **Prefect `@task`** 로 감싸고 `@flow` 로 묶으면, 컨테이너 env 의 `PREFECT_API_URL` 덕분에 그 subprocess 가 **자기 flow run 과 task** 를 server 에 보고해 대시보드에 단계가 보입니다 (orchestrator run 과 별개 flow run, subprocess 라 격리는 유지 — [Appendix H](#appendix-h-prefect-task)).
