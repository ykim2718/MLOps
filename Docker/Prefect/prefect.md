...
- **단일 호스트** — 위 구성은 한 호스트에서 모든 컨테이너가 `mlops` 네트워크를 공유하는 것을 기본으로 합니다. 다른 머신에 dispatcher 를 더 붙이는 방법은 [§4](#4-prefect-worker-container) 의 Concurrency & Scaling 을 참고합니다.
...
- **pool 타입** — 실행 환경을 어떻게 만드느냐로 갈립니다.
  - **Long Term Container** (`process` 타입) — dispatcher **자신의 상시 컨테이너 안에서 subprocess** 로 실행합니다. run 들이 같은 컨테이너를 공유합니다 (단일/소규모: [Appendix C](#appendix-c-long-term-container-work-pool)).
...
worker 는 **두 방식**을 지원합니다. **Short Term Container** (`docker` work pool, 주력) 에서는 worker 가 **dispatcher** 로서 job 마다 Pipeline Flow 컨테이너를 띄웠다 정리하고, **Long Term Container** (`process` work pool) 에서는 worker 가 **자기 상시 컨테이너 안에서 flow 를 직접 실행**합니다 (별도 컨테이너 없음). 이 절은 주력인 **Short Term Container** 설정을 다루며, Long Term Container 설정은 [Appendix C](#appendix-c-long-term-container-work-pool) 를, work pool type 정의는 [Appendix A](#appendix-a-terminology) 를 참고합니다.
...
> **보안 주의** — 도커 소켓 마운트는 dispatcher 에 호스트 도커 전체 제어권 (사실상 root 권한) 을 줍니다. 신뢰된 내부망·스터디 용도로 한정합니다. 더 강한 격리가 필요하면 Kubernetes work pool 을 고려합니다 ([Appendix D](#appendix-d-orchestrator-benchmarking)).
...
처리량을 늘리려면 `--limit` 을 키우거나, **다른 머신에서 dispatcher 를 더 띄워 같은 pool 에 붙입니다** — 그 머신에서 `PREFECT_API_URL` 을 server 로 두고 worker 를 띄우면 polling 시작과 동시에 합류합니다 (추가 dispatcher 는 `CREATE_POOL=false` 로 기존 pool 에 합류, 다른 머신이면 worker compose 의 `networks` 블록을 빼고 `CONTROL_NODE_HOST` 를 server IP 로). 특정 머신 (예: GPU) 전용으로 보내려면 그 머신만 폴링하는 **전용 pool** (`WORK_POOL=docker-gpu`) 로 라우팅합니다.
...
이 한 이미지를 **Short Term 과 Long Term 양쪽에 그대로** 씁니다 — ST (docker pool) 는 job 마다 이 이미지로 컨테이너를 띄우고, LT (process pool) 는 이 이미지를 worker 로 상시 띄웁니다 ([Appendix C](#appendix-c-long-term-container-work-pool)). 그래서 라이브러리를 worker 와 이미지에 중복 설치할 필요가 없습니다.
...
- **저장 위치** — 빌드한 이미지는 빌드를 실행한 호스트의 **로컬 Docker 이미지 스토어**에 저장됩니다 (`docker images` 로 확인). 같은 호스트의 dispatcher 는 같은 데몬을 보므로 그대로 씁니다. 여러 머신에서 쓰려면 레지스트리에 push 후 pull 하거나 각 머신에서 빌드합니다.
...
- **이력 자동 저장** — `@flow` 진입 시 Prefect 가 run 의 상태·로그·파라미터 (`git_commit`·`minio_version` 포함) 를 `prefect` DB 에 자동 기록합니다 (대시보드 Flow Runs). 지표·모델은 팀원 코드가 MLflow 로 로깅하면 함께 남습니다 ([Appendix E](#appendix-e-prefect-task)).
