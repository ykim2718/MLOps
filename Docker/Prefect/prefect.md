...
### Setup Files
  ...
  3) **PIPELINE FLOW** — job 마다 떴다 사라지는 컨테이너 · 직접 빌드
     ...
     prefect deploy --prefect-file pipelineflow-high.yml --name pipelineflow-high   # register a deployment (host shell, once; repeat for -low)
     ```
...
dispatcher 는 **`docker` work pool** 을 polling 해 job 마다 `pipeline_flow` 컨테이너를 띄웠다 정리합니다 — flow 코드는 **그 컨테이너가** 실행하고 dispatcher 자신은 실행하지 않습니다. 이 스택의 `high_performance`·`low_performance` 는 [§2](#work-pool-registration) 에서 `--type docker` 로 등록합니다.
...
### 4.1 Image
  ...
  # pipeline.py — orchestrator (deployment entrypoint); team repo is fetched at runtime into a per-run worktree
  COPY pipeline.py .
  ```
...
### 4.2 Deployment
  ...
  prefect deploy --prefect-file pipelineflow-high.yml --name pipelineflow-high
  ```
...
- **deployment** — flow 를 어떻게 실행할지 묶어 **server DB (`prefect`) 에 저장한 레코드** 입니다. 파일·dict 가 아니라 server 안의 영구 레코드이고, API·UI·`prefect deployment inspect` 에서 **JSON 으로** 보입니다.
  ...
  - **어떻게** — `prefect deploy --prefect-file <yaml> --name <name>` (CLI) 가 yaml 정의를 server API 로 보내 DB 에 등록합니다 ([§4.2](#42-deployment)).
