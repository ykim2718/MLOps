...
1) **PREFECT SERVER** — 제어 노드 1대 · 공식 이미지라 빌드 없음
   ...
   ```
   ├─ docker-compose.server.yml      server container def (port 4200 · mounts high/low.json)
   ├─ run_server.ps1                 start: create network + compose up
   ├─ register_pool.ps1              register work pools (once, after the server is up)
   └─ high.json / low.json           per-tier base job template ( = flow container settings )
   ```
...
2) **PREFECT DISPATCHER** — 작업 머신마다 1대 · 직접 빌드
   ...
   ├─ docker-compose.dispatcher.yml  container def (mounts docker.sock)
...
3) **PIPELINE FLOW** — job 마다 떴다 사라지는 컨테이너 · 직접 빌드
   ...
   ├─ pipeline.py                    orchestrator (COPYd into the image)
   └─ deploy.py                      register deployments (on the server, admin, once)
   ```
   ...
   python deploy.py                                  # register deployments (host shell, once)
   ```
