...
이하 문서에서 **Short Term Container 는 ST**, **Long Term Container 는 LT** 로 줄여 씁니다.
...
| **Prefect Dispatcher** | dispatcher / executor | Short Term (주력): pool 을 polling 해<br>job 마다 Pipeline Flow 컨테이너를 띄웁니다.<br>Long Term: 자기 컨테이너에서 flow 직접 실행. | 상시 |
| **Pipeline Flow** | execution unit | flow (코드) 가 실행되는 곳입니다.<br>Short Term 은 job 마다 뜨는 전용 컨테이너,<br>Long Term 은 Prefect Dispatcher 안 (별도 없음). | Short Term: 일시적<br>Long Term: 상시 |
...
|---------|----------|------|
| `postgres` | `:5432` | Metadata DB. 한 인스턴스에서 `prefect`/`mlflow`/`optuna`/`catalog` 4개 논리 DB 를 운영합니다. |
...
| `prefect_dispatcher` | — | job polling · job dispatch · reporting · cleanup. |

> `postgres`·`minio`·`mlflow` 는 각자 폴더의 compose 로 띄웁니다. 이 문서는 **Prefect server·dispatcher 와 `pipeline_flow` 이미지** 에 집중합니다.

## 2. Execution Architecture

Prefect 실행 모드는 serve mode 와 work pool mode (LT·ST) 이고, 차이는 **누가 코드를 실행하느냐** 입니다.
...
| Work Pool Mode<br>- LT Container | `flow.deploy()`<br>`prefect work-pool create --type process` | dispatcher 프로세스 | run 들이 컨테이너 공유 | dispatcher 환경 | 단일/소규모 |
...
**무엇이 되는지는 pool 의 `--type` 이 정합니다** — `--type docker` 면 ST (job 마다 컨테이너), `--type process` 면 LT (subprocess). dispatcher 는 자기가 polling 하는 pool 의 type 을 따르며 worker 종류도 그에 맞춥니다 (docker worker ↔ docker pool). 이 스택의 `high_performance`·`lower_performance` 는 [§3](#work-pool-registration) 에서 `--type docker` 로 등록했으니 **ST** 입니다.

이 절은 주력인 **ST (docker pool) 전용** dispatcher 설정을 다룹니다 (LT 는 [Appendix E](#appendix-e-long-term-container-work-pool), work pool type 정의는 [Appendix A](#appendix-a-terminology)). 준비물은 **dispatcher compose** 하나입니다 — base job template 등록은 server [§3](#3-prefect-server-container), Pipeline Flow 이미지는 [§5](#5-pipeline-flow-container) 입니다.
...
이 한 이미지를 **Short Term·Long Term 양쪽에 그대로** 씁니다 — ST 는 job 마다 컨테이너로, LT 는 dispatcher 로 상시 띄웁니다 ([Appendix E](#appendix-e-long-term-container-work-pool)). 그래서 라이브러리를 dispatcher 와 이미지에 중복 설치할 필요가 없습니다.
...
## Appendix E. Long Term Container Work Pool

단일 머신·소규모에서는 매 flow 컨테이너를 띄우는 대신 dispatcher **자기 프로세스** 로 실행하는 Long Term Container 가 더 단순합니다. **Short Term 과 같은 `pipeline-flow` 이미지를 dispatcher 로 띄우고** ([§5.1](#51-image)), 라이브러리·orchestrator 는 이미지에 이미 있습니다 (process worker 라 `prefect-docker`·도커 소켓 불필요). 팀 repo 는 ST 와 똑같이 run 마다 `git clone` 으로, 데이터는 MinIO 에서 직접 read 합니다. run 들이 프로세스 공간을 공유하므로 격리는 약합니다. process pool 은 한 번 등록합니다 (`prefect work-pool create <pool> --type process --overwrite` — [§3](#work-pool-registration) 과 같되 type 만 process).

```yaml
# docker-compose.dispatcher.yml (process variant — single/small scale)
services:
  prefect_dispatcher:
    image: pipeline-flow:latest     # the same image as the Short Term run container (§5.1; has prefect)
    env_file:
      - docker-compose.env          # PREFECT_API_URL (the Prefect server API)
    command: prefect worker start --pool ${WORK_POOL:-default} --limit ${WORKER_LIMIT:-8}
