...
> **두 종류의 python 코드를 구분하세요.** 이미지에 굽는 건 아래 **orchestrator (flow)** — git checkout·데이터 준비·실행을 지시하는 **바뀌지 않는 얇은 골격** 뿐입니다. 팀원이 동적으로 바꾸는 **실제 ML 코드 (`train.py` 등)** 는 이미지에 굽지 않고 `git_commit` 으로 매 job 받아 옵니다 (§7). 즉 "이미지에 코드를 등록" 하는 대상은 이 골격이고, 팀원의 코드는 git 으로 동적 전달됩니다.

| Code | What | Where | Changes |
|------|------|-------|---------|
| **Orchestrator (flow)** | git checkout·데이터 준비·실행을 지시하는 골격 | 이미지에 구움 (빌드 시 1회) | 고정 (인프라) |
| **ML payload** (`train.py`) | 팀원의 실제 학습 코드 | `git_commit` 으로 매 job 전달 | 동적 |

flow (orchestrator) 는 **파라미터로 받은 커밋으로 소스를 전환한 뒤 실제 코드를 실행** 하는 얇은 골격입니다. 이 골격 자체는 이미지에 구운 버전으로 고정되고, 바뀌는 payload 코드 (예: `train.py`) 는 `git checkout` 후 **하위 프로세스로** 실행합니다. 각 Step A~D 는 §1 의 컨테이너 내부 흐름과 같습니다.
