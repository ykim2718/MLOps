...
- **deployment = 등급별 등록** — **deployment** (flow 를 어떤 pool·파라미터로 실행할지 server 에 등록한 실행 정의) 은 pool 하나에 바인딩되므로, 같은 flow 를 등급마다 등록해 (`pipeline/high`·`pipeline/low`) job 을 보낼 등급을 고릅니다 (등록 방법은 [§4.2](#42-deploypy)).
...
설치 파일은 세 구성요소 + 공유 env 로 나뉩니다. 각 구성요소의 파일과 실행 명령을 함께 적습니다 (인자는 기본값까지 명시).

1) **PREFECT SERVER** — 제어 노드 1대 · 공식 이미지라 빌드 없음

   ```
   ├─ docker-compose.server.yml      서버 컨테이너 정의 (포트 4200 · high/low.json 마운트)
   ├─ run_server.ps1                 기동: 네트워크 만들고 compose up
   ├─ register_pool.ps1                   work pool 등록 (서버 뜬 뒤 1회)
   ├─ high.json / low.json           pool 의 base job template ( = flow 컨테이너 설정값 )
   └─ docker-pool-template.json      (단일 pool 용 기본형 · 참고)
   ```
   ...
   .\register_pool.ps1 -PoolName high_performance  -TemplateFile high.json -ConcurrencyLimit 16 -ProjectName mlops -Compose docker-compose.server.yml
   .\register_pool.ps1 -PoolName lower_performance -TemplateFile low.json  -ConcurrencyLimit 4  -ProjectName mlops -Compose docker-compose.server.yml
   ```

2) **PREFECT DISPATCHER** — 작업 머신마다 1대 · 직접 빌드

   ```
   ├─ Dockerfile.dispatcher          이미지 레시피 (python + prefect + prefect-docker)
   ├─ docker-compose.dispatcher.yml  컨테이너 정의 (docker.sock 마운트)
   └─ run_dispatcher.ps1             기동: compose up
   ```

   실행:

   ```powershell
   docker build -f Dockerfile.dispatcher -t prefect-dispatcher:latest .    # 이미지 1회 빌드
...
3) **PIPELINE FLOW** — job 마다 떴다 사라지는 컨테이너 · 직접 빌드
   ...
   ```
   ├─ Dockerfile.pipeline_flow       flow 이미지 레시피 (FROM python:3.11.15)
   ├─ requirements.txt               팀 라이브러리 (torch · mlflow · optuna …)
   ├─ pipeline.py                    orchestrator (이미지에 COPY)
   └─ deploy.py                      deployment 등록 (server 에, 관리자 1회)
   ```

   실행:

   ```powershell
   docker build -f Dockerfile.pipeline_flow -t pipeline-flow:latest .   # 이미지 1회 빌드
   python deploy.py                                  # deployment 등록 (호스트 셸, 1회)
   ```

- **공유** — 세 컨테이너가 모두 읽음

   ```
   └─ docker-compose.env             자격증명 · PREFECT_API_URL (gitignore)
   ```
...
**등록에는 dispatcher 정보가 필요 없습니다** — 등록값은 pool 이름·`--type`·base job template 뿐이고, pool 은 dispatcher 와 독립이라 dispatcher 가 0개여도 등록됩니다 (그동안 trigger 된 run 은 `Late` 로 대기). dispatcher 는 나중에 `worker start` 로 그 pool 에 붙습니다 ([§3.2 Container](#32-container)).
...
.\register_pool.ps1 -PoolName high_performance  -TemplateFile high.json -ConcurrencyLimit 16
.\register_pool.ps1 -PoolName lower_performance -TemplateFile low.json  -ConcurrencyLimit 4
```
...
기동은 **`run_dispatcher.ps1`** (dispatcher 런처; 코드는 [Appendix F](#appendix-f-run_dispatcherps1)) 으로 합니다 — `.\run_dispatcher.ps1 -WorkPool <등급>` 으로 머신마다 1회 실행합니다.

run_dispatcher.ps1 → `docker compose up` 으로 컨테이너가 뜨면 `prefect worker start` 가 컨테이너 안에서 실행되고, **그 순간** server 에 자기를 알리며 (heartbeat 시작) 해당 work pool 에 **자동 등록**됩니다 — **polling 시작 = 등록** 이라 별도 절차가 없습니다. heartbeat 가 끊기면 잠시 뒤 **OFFLINE** 으로 바뀝니다. (dispatcher 등록은 deployment 등록과 별개입니다.)
...
Pipeline Flow 는 dispatcher 가 job 마다 띄우는 per-flow 컨테이너입니다. dispatcher 하나가 동시 job 수만큼 **여러 개 (n 개)** 를 띄우며 (상한 `--limit`, 현재 8), 각 컨테이너는 독립입니다. 세 가지를 다룹니다 — 컨테이너가 쓰는 **이미지** ([§4.1](#41-image)), 그 이미지로 무엇을 실행할지 server 에 등록하는 **`deploy.py`** ([§4.2](#42-deploypy)), 컨테이너 안에서 도는 **orchestrator `pipeline.py`** ([§4.3](#43-pipelinepy)). dispatcher 자신은 flow 를 실행하지 않으므로 flow 는 **별도 이미지** 를 쓰며 ([§3.1](#31-image)), 팀 라이브러리는 이 flow 이미지에만 둡니다.

> **용어 — orchestrator (flow) 는 무엇인가** — 이미지도 컨테이너도 아니라 **python 코드** 입니다 (`pipeline.py` 안의 `@flow` 함수 `pipeline`). 관계: `Dockerfile.pipeline_flow` (레시피) → `pipeline-flow:latest` (이미지, `pipeline.py` 를 품음) → 컨테이너 (이미지의 실행 인스턴스) 안에서 Prefect 가 그 `@flow` 함수를 호출합니다.
...
server 에 **deployment** (어떤 flow 를·어느 pool 에서·어떤 이미지로 실행할지 — [Appendix A](#appendix-a-terminology)) 를 등록하는 스크립트입니다. `from_source(entrypoint="pipeline.py:pipeline")` 로 **deployment entrypoint** 를 명시하면, 그 문자열이 **server 의 deployment 레코드 (`prefect` DB)** 에 저장됩니다 (`prefect deployment inspect` / UI 에서 확인). 여기서 `pipeline.py` 는 **flow 이미지에 구워진 orchestrator** — `Dockerfile.pipeline_flow` 가 컨테이너 `/opt` 에 COPY 한 `Prefect/pipeline.py` 입니다 (팀 코드가 아니며, 경로도 그 `/opt` 기준). 콜론 뒤 `pipeline` 은 `pipeline.py` 안 **`@flow` 데코레이터가 붙은 함수** 입니다. **플랫폼·관리자가 1회** 하는 일이고 (팀원·등급 구분은 deployment 이름·pool), 팀원이 작성하는 팀 스크립트 (`train.py`) 에는 넣지 않습니다.

```python
# deploy.py — register the deployments (admin, once); entrypoint is given as "<file>:<flow function>".
...
이 등록은 **제어 노드 호스트 셸에서 (컨테이너 밖) 관리자가 1회** 실행합니다 — `prefect` 가 깔린 곳에서 `pipeline.py` 가 있는 폴더로 가, `PREFECT_API_URL` 을 server 로 두고 돌립니다. deploy.py 는 DB 에 직접 쓰지 않고 server API 로 등록을 보냅니다 (server 가 Postgres 에 저장하며, 등급마다 `high`·`low` 등록합니다).

```powershell
cd <pipeline.py 가 있는 폴더>
$env:PREFECT_API_URL = "http://localhost:4200/api"   # 다른 머신이면 http://<server IP>:4200/api
python deploy.py                                       # server 에 deployment 등록 (1회)
```
...
orchestrator (`pipeline.py`) 는 **"커밋 받아 → 팀원 코드 실행"** 만 하는 얇은 python 골격 (`@flow` 함수) 으로, [§4.1](#41-image) 이미지에 구워집니다. **플랫폼·관리자가 관리하는 공유·고정 골격** 이라 팀원이 작성하지 않습니다 — 팀원은 자기 스크립트 (`train.py` 등) 만 작성해 `payload` 파라미터로 고릅니다 (`payload` 는 팀 스크립트일 뿐, deployment entrypoint 인 `pipeline.py` 와 다릅니다 — `pipeline.py` 를 넣으면 orchestrator 가 자기 자신을 다시 부릅니다).
...
flow 컨테이너에서 GPU 를 쓰려면 호스트에 NVIDIA 드라이버·nvidia-container-toolkit 이 있고 base job template 에서 GPU 를 요청해야 합니다 ([§2 Work Pool Registration](#work-pool-registration)). torch 의 CUDA 휠은 런타임을 번들하므로 호스트 드라이버가 최신이면 동작하며, 버전이 안 맞으면 베이스 이미지를 `nvidia/cuda` 계열로 교체합니다. GPU job 은 보통 무거우므로 그 등급 dispatcher 의 `--limit` 을 1~2 로 낮춰 동시 실행을 제한합니다.
...
- **deployment** — flow 를 어떻게 실행할지 묶어 **server DB (`prefect`) 에 저장한 레코드** 입니다. 파일·dict 가 아니라 server 안의 영구 레코드이고, API·UI·`prefect deployment inspect` 에서 **JSON 으로** 보입니다.
  ...
  - **어떻게** — `flow.from_source(...).deploy(...)` (Python) 또는 `prefect deploy` (CLI, `prefect.yaml` 을 읽음) 가 server API 로 보내 DB 에 등록합니다 ([§4.2](#42-deploypy)).
  - **사용** — 코드를 다시 안 봐도 이름 `<flow>/<deployment>` 로 run 을 trigger 합니다 (`prefect deployment run "pipeline/high" -p payload=train.py` · UI · 스케줄). 그러면 dispatcher 가 그 정의대로 `pipeline_flow` 컨테이너를 띄웁니다.
  - 저장된 모습 (`prefect deployment inspect "pipeline/high"`):

    ```json
    { "name": "high", "flow_name": "pipeline", "entrypoint": "pipeline.py:pipeline",
