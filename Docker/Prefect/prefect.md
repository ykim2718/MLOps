...
### Server Setup
  ...
  - `run_server.ps1` (코드는 [Appendix D](#appendix-d-run_serverps1)) — 네트워크 생성과 `docker compose up` 을 한 번에 처리합니다. `-ProjectName` 으로 프로젝트명을, `-Yaml` 로 띄울 compose 파일을, `-Network` 로 붙을 공유 네트워크를 받습니다.
...
### 3.1 Image
  ...
  - `docker build -f Dockerfile.dispatcher -t prefect-dispatcher:latest .` — `-f` 로 dockerfile 을, `-t` 로 이미지 태그를, `.` 로 빌드 컨텍스트를 지정해 1회 빌드합니다.
...
### 4.1 Image
  ...
  - `FROM python:3.11.15` + `apt-get install git` — 베이스 이미지에 런타임 `git clone` 용 git 을 더합니다.
  - `COPY requirements.txt` → `pip install` — 팀 라이브러리를 설치합니다 (코드보다 먼저 복사해 레이어 캐시를 살립니다).
  - `COPY pipeline.py` — orchestrator 만 이미지에 굽습니다. 팀 코드는 런타임에 `git clone` 으로 받습니다.

  실행 명령은 다음과 같습니다.

  - `docker build -f Dockerfile.pipeline_flow -t pipeline-flow:latest .` — `-f` 로 dockerfile 을, `-t` 로 이미지 태그를, `.` 로 빌드 컨텍스트를 지정해 셋업 때 1회 빌드합니다.
...
### 4.2 deploy.py
  ...
  ```powershell
  cd <folder containing pipeline.py>
  $env:PREFECT_API_URL = "http://localhost:4200/api"   # on another machine, http://<server IP>:4200/api
  python deploy.py                                       # register the deployment on the server (once)
  ```
