...
| Case | Trigger | Trigger Location | Execution<br>Mode | Worker<br>Location | Execution<br>Location | Credentials | Reference |
|------|---------|------------------|----------------|-----------------|--------------------|-------------|-----------|
...
| **D** | user | Client | work pool | Shared<br>Worker<br>Node | Worker<br>machine | 불필요 | [Appendix D](#appendix-d-additional-worker-attachment) |
| **E** | user | Client | work pool | Dedicated<br>Worker<br>Node | Client | 필요 | [Appendix D](#appendix-d-additional-worker-attachment) |
...
붙이는 방식은 **어느 work pool 에 붙이느냐**에 따라 두 가지입니다.

### ① Shared — `default` work pool 공유

기존 **`default` pool** 에 worker 를 더 붙입니다. 그 pool 을 바라보는 모든 worker (중앙 Worker Node + 새로 붙은 worker) 가 job 을 **나눠 가져 처리량이 늘어납니다.** 다만 **먼저 비는 worker 가 가져가므로 어느 머신에서 돌지는 고를 수 없습니다** (단순 분산용). 이것이 [Execution Scenarios](#execution-scenarios) 표의 **Case D** 입니다.
...
이렇게 하면 중앙 Worker Node 와 **로컬 worker 가 같은 `default` pool** 을 함께 바라보게 되고, job 이 가용한 worker 로 분산 실행됩니다. 여러 대를 붙여도 되며 (OS·하드웨어 무관), 모두 `default` pool 을 공유합니다.

### ② Dedicated — 전용 pool/queue (특정 머신 고정)

특정 머신에서만 job 을 돌리려면 (예: 내 GPU), 그 머신 전용 경로를 만들어 라우팅합니다. ①처럼 `default` pool 에 붙이면 분산되어 머신을 고정할 수 없으므로, 둘 중 하나를 씁니다.
...
user 가 이렇게 자기 컴퓨터의 worker 로 job 을 고정한 것이 [Execution Scenarios](#execution-scenarios) 표의 **Case E** 이며, 그 머신엔 자격증명을 직접 공급해야 합니다 ([Credentials](#credentials)).
...
어느 pool 로 갈지는 **deployment 의 `work_pool_name`** 이 정하며, Python 으로 지정합니다 (process pool 이라 이미지 빌드는 불필요).
...
# 등록 — 전용 pool 로 바인딩 (worker 에 코드가 마운트돼 있으므로 from_source 로 경로만 가리킨다)
full_pipeline.from_source(
    source="/app",                        # flow 코드 위치 (repo 경로 또는 git URL)
    entrypoint="flow.py:full_pipeline",   # 파일:flow함수
).deploy(
    name="member2-dep",
    work_pool_name="member2-pool",        # ← 이 pool 로만 간다 (Case E)
    # work_queue_name="gpu",              # (선택) pool 안 특정 queue 로 더 좁힘
)

# 실행 — 그 deployment(= member2-pool) 로 run 을 만든다
run_deployment(name="full-pipeline/member2-dep")
```

`work_pool_name` 을 `default` 로 두면 분산 (Case D), `member2-pool` 이면 그 머신에 고정 (Case E) 됩니다.
