...
| **2. Prefect Worker** | dispatcher | Short Term Container work pool 을 polling 해 job 마다 Pipeline Flow 컨테이너를 띄우고, 끝나면 정리합니다. 코드는 실행하지 않습니다. | 상시 |
| **3. Pipeline Flow** | execution container | 코드를 실행하는 컨테이너입니다. 받은 커밋으로 git checkout 한 뒤 실제 코드를 실행합니다 — 주력은 **Short Term Container** (docker pool, job 마다 생성·파괴), 단일/소규모는 **Long Term Container** (process pool, 상시 공유) 입니다. | Short Term: 일시적 (job 1개당 1개) / Long Term: 상시 |
...
| `prefect_worker` | — | Short Term Container work pool 을 polling 해 job 마다 Pipeline Flow 컨테이너를 띄우는 dispatcher 입니다. |
