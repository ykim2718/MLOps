...
- **deployment = 등급별 등록** — **deployment** (flow 를 어떤 pool·파라미터로 실행할지 server 에 등록한 실행 정의) 은 pool 하나에 바인딩되므로, 같은 flow 를 등급마다 등록해 (`pipeline/pipelineflow-high`·`pipeline/pipelineflow-low`) job 을 보낼 등급을 고릅니다 (등록 방법은 [§4.2](#42-deployment)).
...
### Setup Files
  ...
  1) **PREFECT SERVER** — 제어 노드 1대 · 공식 이미지라 빌드 없음
  ...
  3) **PIPELINE FLOW** — job 마다 떴다 사라지는 컨테이너 · 직접 빌드
     ...
     prefect deploy --file pipelineflow-high.yml --name pipelineflow-high   # register a deployment (host shell, once; repeat for -low)
     ```
  ...
  services:
    prefect_server:
      ...
      volumes:
        ...
        - ./docker-pool-template-low.json:/templates/docker-pool-template-low.json:ro     # base job template for the lower_performance pool
      networks:
        - mlops
...
### Work Pool Registration
  ...
  work pool 은 **server 에 저장되는 메타데이터 (컨테이너 아님)** 라, server 가 뜨면 한 번 등록합니다. 등록된 pool 은 server DB 에 남아 이후 dispatcher 들이 polling 으로 접근하므로 ([§3](#3-prefect-dispatcher-container)), dispatcher 쪽엔 pool 생성 단계가 없습니다.

  **등록에는 dispatcher 정보가 필요 없습니다** — 등록값은 pool 이름·`--type`·base job template 뿐이고, pool 은 dispatcher 와 독립이라 dispatcher 가 0개여도 등록됩니다 (그동안 trigger 된 run 은 `Late` 로 대기). dispatcher 는 나중에 `prefect worker start` 로 그 pool 에 붙습니다 ([§3.2 Container](#32-container)).
  ...
  **Base job template** — pool 이 띄우는 모든 `pipeline_flow` 컨테이너의 공통 설정입니다. flow 컨테이너는 dispatcher 의 마운트·네트워크를 상속하지 않으므로 **`PREFECT_API_URL` 과 네트워크를 여기서 명시** 합니다. 등급별로 `docker-pool-template-high.json`·`docker-pool-template-low.json` 두 벌을 두며 (`job_configuration` 은 같고 `variables` 의 `mem_limit` default 만 등급별로 다릅니다 — 아래는 high 예시, low 는 표 참고), 위 server compose 가 이를 server 컨테이너에 마운트해 둡니다.
  ...
  - `image` — flow 컨테이너로 쓸 Pipeline Flow 이미지 ([§4.1](#41-image)). 태그 (`pipeline-flow:latest`) 가 곧 **런타임 버전** (라이브러리 + orchestrator) 입니다.
...
## 3. Prefect Dispatcher Container
...
dispatcher 는 **`docker` work pool** 을 polling 해 job 마다 `pipeline_flow` 컨테이너를 띄웠다 정리합니다 — flow 코드는 **그 컨테이너가** 실행하고 dispatcher 자신은 실행하지 않습니다. 이 스택의 `high_performance`·`lower_performance` 는 [§2](#work-pool-registration) 에서 `--type docker` 로 등록합니다 (work pool type 정의는 [Appendix A](#appendix-a-terminology)).

준비물은 **dispatcher compose** 하나입니다 — base job template 등록은 server [§2](#2-prefect-server-container), Pipeline Flow 이미지는 [§4](#4-pipeline-flow-container) 입니다.
...
### 3.1 Image
  ...
  ```powershell
  # (once) build the dispatcher image (prefect + prefect-docker baked in).
  docker build -f Dockerfile.dispatcher -t prefect-dispatcher:latest .
  ```

  - `docker build -f Dockerfile.dispatcher -t prefect-dispatcher:latest .` — 1회 빌드합니다.
  - `-f` — 빌드할 dockerfile.
  - `-t` — 이미지 태그.
  - `.` — 빌드 컨텍스트.
...
### 3.2 Container
  ...
  - `command` — `prefect worker start` 만 합니다. prefect·prefect-docker 는 **이미지에 구워져** 있고 `PREFECT_API_URL` 은 env_file 이 주므로, 부팅 때 설치·export 가 없습니다 (`bash -c` 도 불필요). pool 생성 단계도 없습니다 (server [§2](#2-prefect-server-container) 가 이미 등록). `WORK_POOL`·`WORKER_LIMIT` 는 `docker compose up` 시 셸에서 읽는 변수입니다.
  ...
  **머신마다 실행** — 같은 compose 를 각 컴퓨터에서 자기 등급 `WORK_POOL` 로 띄웁니다. pool 이 server 에 이미 있으니 (§2) dispatcher 는 polling 만 하며, 등급별 첫 머신/추가 머신 구분이 없습니다.
  ...
  > **보안 주의** — 도커 소켓 마운트는 dispatcher 에 호스트 도커 전체 제어권 (사실상 root) 을 줍니다. 신뢰된 내부망·스터디 용도로 한정하고, 더 강한 격리는 Kubernetes work pool 을 고려합니다 ([Appendix G](#appendix-g-orchestrator-benchmarking)).
...
### 3.4 Verification
  ...
  server 에서 dispatcher 가 ONLINE 인지, pool 이 served 인지 확인합니다.
...
## 4. Pipeline Flow Container

Pipeline Flow 는 dispatcher 가 job 마다 띄우는 per-flow 컨테이너입니다. dispatcher 하나가 동시 job 수만큼 **여러 개 (n 개)** 를 띄우며 (상한 `--limit`, 현재 8), 각 컨테이너는 독립입니다. 세 가지를 다룹니다 — 컨테이너가 쓰는 **이미지** ([§4.1](#41-image)), 그 이미지로 무엇을 실행할지 server 에 등록하는 **deployment** ([§4.2](#42-deployment)), 컨테이너 안에서 generic flow orchestrator 역할을 하는 `pipeline.py` ([§4.3](#43-pipelinepy)). dispatcher 자신은 flow 를 실행하지 않으므로 flow 는 **별도 이미지** 를 쓰며 ([§3.1](#31-image)), 팀 라이브러리는 이 flow 이미지에만 둡니다. 실행이 server UI 에 어떻게 보이는지는 [§4.4](#44-prefect-ui) 입니다.

### 4.1 Image

  job 마다 뜨는 컨테이너의 python 환경입니다. **라이브러리와 orchestrator (`pipeline.py`) 만** 굽습니다. 팀 코드는 런타임에 `git fetch` + `git worktree` 로 받아 (`git_commit` 으로 특정 커밋에 고정) 컨테이너의 사설 디렉터리에 펼칩니다. 이미지가 한 번 빌드로 고정되어 모두 같은 런타임을 씁니다.
  ...
  WORKDIR /work
  COPY requirements.txt .               # required: prefect, boto3
  RUN pip install --no-cache-dir -r requirements.txt

  COPY pipeline.py .                    # orchestrator (deployment entrypoint); team repo is fetched at runtime into a per-run worktree
  ```

  - `FROM python:3.11.15` + `apt-get install git` — 베이스 이미지에 런타임 `git fetch`·`git worktree` 용 git 을 더합니다.
  ...
  - `COPY pipeline.py` — orchestrator 만 이미지에 굽습니다. 팀 코드는 런타임에 `git fetch` + `git worktree` 로 받습니다.

  #### Execution Command

  ```powershell
  # Build once at setup (libraries + orchestrator only). The team repo is fetched at runtime per run.
  docker build -f Dockerfile.pipeline_flow -t pipeline-flow:latest .
  ```

  - `docker build -f Dockerfile.pipeline_flow -t pipeline-flow:latest .` — 셋업 때 1회 빌드합니다.
  - `-f` — 빌드할 dockerfile.
  - `-t` — 이미지 태그.
  - `.` — 빌드 컨텍스트.

  **GPU** — 이 이미지로 GPU 를 쓰려면 `requirements.txt` 의 torch 를 CUDA 휠로 설치합니다 (CUDA 런타임이 휠에 번들되어 호스트 드라이버만 맞으면 동작). 더해 호스트에 NVIDIA 드라이버·nvidia-container-toolkit 을 두고, base job template 에서 GPU 를 요청합니다 ([§2 Work Pool Registration](#work-pool-registration)). 드라이버와 CUDA 버전이 안 맞으면 베이스 이미지를 `nvidia/cuda` 계열로 바꿉니다. GPU job 은 무거우므로 그 등급 dispatcher 의 `--limit` 을 1~2 로 낮춰 동시 실행을 제한합니다.

### 4.2 Deployment

  server 에 **deployment** (어떤 flow 를·어느 pool 에서·어떤 이미지로 실행할지 — [Appendix A](#appendix-a-terminology)) 를 등록합니다. **플랫폼·관리자가 1회** 하는 일이며 (팀원·등급 구분은 deployment 이름·pool), 팀원이 작성하는 학습 스크립트 (`my_flow.py`) 와는 무관합니다.
  ...
  - `entrypoint: pipeline.py:pipeline` — 실행할 flow 를 `<파일>:<@flow 함수>` 로 가리킵니다 (어떻게 `pipeline.py` 가 되는지는 [§4.3](#43-pipelinepy)).
  - `work_pool.name: high_performance` — 이 deployment 가 제출될 work pool 입니다.
  - `job_variables.image: pipeline-flow:latest` — flow 를 띄울 이미지입니다 ([§4.1](#41-image)).
  - `parameters.payload: my_flow.py` — flow 파라미터 기본값입니다 (`git_repo`·`git_commit`·`minio_key`·`member` 는 trigger 때 줍니다).

  #### Execution Command

  ```powershell
  cd <folder containing pipeline.py and pipelineflow-high.yml>
  ...
  - `prefect deploy --file pipelineflow-high.yml --name pipelineflow-high` — yaml 정의를 server 에 등록합니다.
  - `--file` — 정의 파일.
  - `--name` — 등록할 deployment.

  이 등록은 **제어 노드 호스트 셸 (컨테이너 밖) 에서 관리자가 1회** 실행합니다 — `prefect` 가 깔린 곳에서 `PREFECT_API_URL` 을 server 로 두고 돌립니다. `prefect deploy` 는 DB 에 직접 쓰지 않고 server API 로 등록을 보냅니다 (server 가 Postgres `prefect` DB 에 저장). 등급마다 `pipelineflow-high`·`pipelineflow-low` yaml 로 두 벌 등록합니다.

### 4.3 pipeline.py

  orchestrator (`pipeline.py`) 는 **"커밋 받아 → 팀원 코드 실행"** 만 하는 얇은 python 골격 (`@flow` 함수) 으로, [§4.1](#41-image) 이미지에 구워집니다. **플랫폼·관리자가 관리하는 공유·고정 골격** 이라 팀원이 작성하지 않습니다 — 팀원은 자기 학습 스크립트 (`my_flow.py` 등) 만 작성해 `payload` 파라미터로 지정합니다.

  [§4.2](#42-deployment) 의 deployment 가 entrypoint 를 **`pipeline.py:pipeline`** 로 가리킵니다. 이 문자열은 server 의 deployment 레코드 (`prefect` DB) 에 저장되고, dispatcher 가 띄운 컨테이너 안에서 Prefect 런타임이 이미지 작업 디렉터리 (`/work`, `Dockerfile.pipeline_flow` 가 `pipeline.py` 를 COPY 한 곳) 기준으로 `pipeline.py` 를 import 해 콜론 뒤 **`@flow` 함수 `pipeline`** 을 run 파라미터 (`git_repo`·`git_commit`·`minio_key`·`minio_bucket`·`minio_version`·`member`·`payload`) 와 함께 호출합니다. 그래서 deployment entrypoint 가 곧 이 `pipeline.py` 입니다.

  `pipeline` 함수에 전달한 run 파라미터 **값** 은 **trigger 할 때** 지정합니다 — trigger 주체는 보통 **팀원** (또는 스케줄·automation) 입니다. 팀원이 자기 머신·CI 에서 CLI `prefect deployment run "pipeline/pipelineflow-high" -p git_repo=… -p git_commit=… -p minio_key=… -p member=…` 을 실행하거나, server UI 의 Run 폼, 스케줄·automation, 또는 `run_deployment(name, parameters={…})` 로 trigger 합니다 (CLI 는 [Appendix B](#appendix-b-prefect-cli)).

  trigger 의 **시작점** 은 누가 어떤 길로 부르든 결국 **server 의 Prefect API 에 run 생성 요청을 보내는 것** 이고, server 가 그 요청을 work pool 에 얹으면 dispatcher 가 집어 `pipeline_flow` 컨테이너를 띄웁니다. 그래서 **`prefect` CLI 만** 써야 하는 건 아닙니다 — `run_deployment(...)` 은 **python (prefect SDK) 으로** trigger 하는 길이고, CLI·UI·스케줄과 똑같이 같은 API 를 호출합니다. 다만 어느 길이든 server 에 **등록된 deployment** 를 부르는 것이라, server 를 거치지 않는 (순수 python 단독) trigger 는 없습니다. Prefect 는 `pipeline` 시그니처에서 만든 **파라미터 스키마** 로 그 값을 검증·저장하고, 컨테이너의 런타임이 `pipeline(**parameters)` 로 호출합니다 (코드가 아니라 값만 넘어갑니다).

  `pipeline.py` 가 run 마다 만드는 폴더 구조입니다 (끝나면 통째로 삭제).

  ```text
  /tmp/run-<rand>/                 # per-run temp dir (base; removed after the run)
  ├─ repo/                         # git init + remote add origin + fetch origin (full history in .git)
  ├─ src/                          # git worktree add --detach src <git_commit> (team repo at the commit)
  │  ├─ my_flow.py                 # payload — the team's entry (run: python my_flow.py --data ../data ...)
  │  └─ ...                        # the rest of the team repo at <git_commit>
  └─ data/                         # MinIO download target (bucket/key[@version] → here)
  ...
  - **팀원별 repo** — `git_repo` 가 **flow 파라미터** 라 deployment 마다 다른 repo 를 기본값으로 등록할 수 있습니다. 팀원은 각자 repo·커밋을 쓰고, run 마다 사설 worktree 에 펼쳐져 서로 간섭하지 않습니다. Prefect 가 `git_repo`·`git_commit` 을 run 파라미터로 자동 기록해 재현·lineage 가 남습니다.
  - **데이터 준비** — `pipeline.py` 가 MinIO 에서 `minio_bucket`/`minio_key` (버전 지정 시 `minio_version`) 객체를 `data/` 로 미리 내려받고 `--data` 로 경로를 넘깁니다. 접속 자격증명 (`MINIO_ENDPOINT`/`MINIO_ACCESS_KEY`/`MINIO_SECRET_KEY`) 은 [§5](#5-credentials) 의 Prefect Secret 으로 받습니다. 팀원 코드는 자격증명·다운로드를 각자 짤 필요 없이 `--data` 폴더의 파일을 읽기만 하면 됩니다 (`pipeline.py` 가 `boto3` 로 받으므로 flow 이미지에 `boto3` 가 있어야 합니다 — [§4.1](#41-image)).
  - **자유로운 코드** — `payload` 로 팀원이 자기 스크립트를 지정하므로 코드를 정해진 틀에 맞출 필요가 없습니다. 입력은 CLI 인자 (`--git_repo`·`--git_commit`·`--member`·`--data`) 로 받으므로, 팀원 스크립트는 `argparse` 로 그 값만 읽으면 됩니다.
  - **데이터 이력** — `minio_bucket`·`minio_key`·`minio_version` 이 **flow 파라미터** 라서 Prefect 가 run 마다 입력값을 `prefect` DB 에 자동 저장합니다 (어느 버킷·객체·버전을 썼는지 lineage 로 남습니다).
  - **crash 확인** — payload 가 0 이 아닌 코드로 끝나면 `subprocess.run(check=True)` 가 `CalledProcessError` 를 던지고, `pipeline` 가 `member@commit` 을 단 에러를 run 로그에 남긴 뒤 다시 raise 해 run 이 **Failed** 로 표시됩니다. payload 의 stdout·stderr 는 실행 중 이 run 의 로그로 흘러 들어가므로, 팀원은 자기 이름이 붙은 run (`alice@a1b2c3d`) 의 **Logs** 에서 crash 원인을 봅니다. payload 가 `@task` 를 쓰면 자기 flow run ([§4.4](#44-prefect-ui)) 에서 **어느 단계** 가 깨졌는지까지 보입니다.
  - **이력 자동 저장** — `@flow` 진입 시 Prefect 가 run 의 상태·로그·파라미터를 자동 기록합니다. 지표·모델은 팀원 코드가 MLflow 로 로깅하면 함께 남습니다 ([Appendix H](#appendix-h-prefect-task)).

### 4.4 Prefect UI

  server 대시보드 (`http://<Host IP>:4200`) 에서 deployment·run·task 가 어떻게 보이는지입니다.

  - **Deployments** — `<flow_name>/<deployment_name>` 로 나열됩니다 (예: `pipeline/pipelineflow-high`·`pipeline/pipelineflow-low`). flow 이름은 `@flow(name="pipeline")`, deployment 이름은 yaml 의 `name` 입니다 ([Appendix A](#appendix-a-terminology)).
  - **Flow Runs** — trigger 된 run 이 `flow_run_name` 으로 나열됩니다. `member` 가 들어가 같은 deployment 아래에서 `alice@a1b2c3d` 처럼 **누구의 run 인지** 구분됩니다 ([§4.3](#43-pipelinepy) 의 `flow_run_name`). `pipeline.py` 가 `member`·`git_commit` 을 payload 에 CLI 인자로 넘기므로 팀 flow run 도 같은 이름을 씁니다.
  - **Tasks** — 팀 payload 가 단계 (dp·fe·train·test) 를 **`@task`** 로 감싸고 `@flow` 로 묶으면, 컨테이너 env 의 `PREFECT_API_URL` 덕분에 그 subprocess 가 **자기 flow run 과 task** 를 보고해 단계가 보입니다 (orchestrator run 과 **별개 flow run**, subprocess 라 격리 유지 — [Appendix H](#appendix-h-prefect-task)).
  - **Parameters · State · Logs** — run 마다 입력 파라미터 (`git_repo`·`git_commit`·`minio_key`·`member`)·상태·로그가 자동 기록되어 (UI 의 Flow Run → Parameters), 같은 파라미터로 재실행 (재현) 할 수 있습니다.

  job 하나가 trigger 되면 대시보드에 다음처럼 보입니다.

  ```text
  Deployments
    pipeline/pipelineflow-high     high_performance     # per-tier registration (§4.2)
    pipeline/pipelineflow-low      lower_performance
...
설정 값은 **세 곳** 으로 나뉘고 서로 겹치지 않습니다 — ① server·dispatcher 인프라 값 (backend DB·Control Node 주소) 은 `docker-compose.env`, ② `pipeline_flow` 컨테이너의 **비-secret 부트스트랩** (`PREFECT_API_URL`·`mem_limit` 등) 은 **base job template** (§2), ③ **run 코드용 자격증명** (MinIO·DB) 만 **Prefect Secret** 입니다. 따라서 base job template 에 적은 값 (`PREFECT_API_URL` 등) 은 **Secret 에 넣지 않습니다** (비밀이 아니고 `docker inspect` 로 보여도 무방). dispatcher 는 자격증명을 들지 않습니다.
...
### Prefect Secret
  ...
  # Save minio-secret-key / minio-endpoint / catalog-dsn / optuna-dsn the same way.
  ```
  ...
  > flow 컨테이너는 base job template 의 `PREFECT_API_URL` 로 server 에 연결돼야 Secret 을 받습니다 ([§2 Work Pool Registration](#work-pool-registration)). `mlflow`·`prefect` DB 는 사용자 코드가 직접 접속하지 않으므로, 사용자 role 에는 `catalog`·`optuna` 권한만 있으면 됩니다.

## 6. Job Triggering

등록된 deployment 를 실제로 돌리는 (trigger) 방법은 여러 가지지만, 결국 모두 **server 의 Prefect API 에 "flow run 생성" 요청을 보내는 것**입니다 — 코드가 아니라 **deployment 이름 + 파라미터 값** 만 보냅니다. **trigger 인터페이스 (CLI·SDK) 는 실행 모드와 무관하게 같고**, 실제 실행 주체는 **실행 모드** 가 정합니다 — 이 스택의 **work pool mode** (server 가 run 을 work pool 에 얹고 dispatcher 가 `pipeline_flow` 컨테이너를 띄워 그 안에서 `pipeline(**parameters)` 실행, [§4.3](#43-pipelinepy)) 와 단일 머신 대안인 **serve mode** ([§6.3](#63-serve-mode) · [Appendix C](#appendix-c-execution-architecture)) 입니다. 그래서 아래 §6.1·§6.2 는 두 모드 공통의 trigger 인터페이스이고, §6.3 이 serve mode 의 차이를 다룹니다.

> ⚠️ `pipeline(...)` 함수를 파이썬에서 직접 호출하는 것은 trigger 가 **아닙니다** — server·work pool 을 거치지 않고 그 자리에서 로컬 실행되어 컨테이너 격리·lineage 가 없습니다. 아래 [§6.2](#62-python-sdk) 는 반드시 `run_deployment` 를 말합니다.
...
### 6.1 Prefect CLI
  ...
  prefect deployment run "pipeline/pipelineflow-high" `
    -p git_repo=https://github.com/team/repo.git -p git_commit=a1b2c3d -p minio_key="SYDNEY/Bennelong Point" -p member=alice
  ```
...
### 6.2 Python SDK
  ...
  flow_run = run_deployment(                                                   # ask the server to create a flow run
      ...
      parameters={"git_repo": "https://github.com/team/repo.git",
                  "git_commit": "a1b2c3d", "minio_key": "SYDNEY/Bennelong Point", "member": "alice"},
  )
...
### 6.3 Serve Mode
  ...
  work pool·dispatcher·이미지 빌드 없이 `flow.serve(name=…)` **한 프로세스가 deployment 등록과 실행을 겸하는** 단일 머신·소규모 대안입니다 ([Appendix C](#appendix-c-execution-architecture)).
  ...
  - **등록+실행** — `flow.serve(...)` 한 줄이 deployment 등록과 실행 프로세스를 겸합니다 (`prefect deploy`·dispatcher·이미지 빌드 불필요).
  - **trigger** — serve 프로세스는 상시 떠 있으므로 **별도 터미널에서** trigger 하며, 방법은 **§6.1·§6.2 와 똑같습니다** (`prefect deployment run "pipeline/pipelineflow-serve"` · `run_deployment(...)`). served 프로세스가 그 run 을 자기 안에서 실행합니다.
  ...
  | Aspect | Work Pool (this stack) | Serve Mode |
  |--------|------------------------|------------|
  | 등록 | `prefect deploy --file pipelineflow-*.yml` | `flow.serve(name=…)` |
  | 실행 주체 | dispatcher 가 띄운 `pipeline_flow` 컨테이너 | serve 프로세스 자신 |
  | 격리 | run 마다 컨테이너 | 없음 (한 프로세스) |
  | 적합 | 다수 팀원·동시 실행·격리 | 단일 머신·소규모 |
...
- **Pipeline Flow** — dispatcher 가 job 마다 띄우는 일시적 실행 컨테이너입니다. 받은 repo·커밋을 `git fetch` + `git worktree` 로 펼친 뒤 코드를 실행하고 끝나면 파괴됩니다.
...
- **deployment** — flow 를 어떻게 실행할지 묶어 **server DB (`prefect`) 에 저장한 레코드** 입니다. 파일·dict 가 아니라 server 안의 영구 레코드이고, API·UI·`prefect deployment inspect` 에서 **JSON 으로** 보입니다.
  ...
  - **어떻게** — `prefect deploy --file <yaml> --name <name>` (CLI) 가 yaml 정의를 server API 로 보내 DB 에 등록합니다 ([§4.2](#42-deployment)).
...
- **§3 Dispatcher**
  - `prefect work-pool get-default-base-job-template --type docker` — 도커 dispatcher 의 기본 base job template 을 출력합니다 (§3.1).
  - `prefect worker start --pool <name> [--limit N]` — dispatcher 를 기동해 그 pool 을 polling 하며 job 을 실행합니다 (§3.2).
  - `prefect work-pool set-concurrency-limit <pool> <N>` — pool 전체 동시 실행 상한을 설정합니다 (§3.3).
- **§4 Pipeline Flow**
  - `prefect deploy` (또는 `flow.deploy(...)`) — deployment 를 등록합니다 (§4.2).
  - `prefect deployment run "<flow>/<deployment>" -p <key>=<value>` — 등록된 deployment 를 파라미터와 함께 trigger 합니다 (§4.3).
...
제어 노드에서 Prefect server compose 스택을 띄우는 기동 스크립트입니다 ([§2 Server Setup](#server-setup)). 공유 `mlops` 네트워크가 없으면 만들고 `docker-compose.server.yml` 을 올립니다. work pool 등록은 별도입니다 (`register_pool.ps1` — [Appendix E](#appendix-e-register_poolps1)).
...
server 에 work pool 을 등록 (또는 갱신) 하는 스크립트입니다 ([§2 Work Pool Registration](#work-pool-registration)).
...
각 dispatcher 머신에서 dispatcher compose 스택을 띄우는 기동 스크립트입니다 ([§3.2](#32-container)). server 기동과 work pool 등록은 별도입니다 (server 는 [Appendix D](#appendix-d-run_serverps1), pool 은 `register_pool.ps1` — [Appendix E](#appendix-e-register_poolps1)).
