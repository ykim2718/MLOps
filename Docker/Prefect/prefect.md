...
| **3. Pipeline Flow** | execution unit | flow (코드) 가 실행되는 곳입니다.<br>Short Term 은 job 마다 뜨는 전용 컨테이너,<br>Long Term 은 Prefect Worker 안 (별도 없음). | Short Term: 일시적<br>Long Term: 상시 |
...
dispatcher 가 띄우는 모든 Pipeline Flow 컨테이너의 공통 설정입니다. flow 컨테이너는 dispatcher 의 마운트·네트워크를 상속하지 않으므로 **`PREFECT_API_URL` 과 네트워크를 여기서 명시** 해야 합니다.
    ...
    "properties": {
      ...
      "env":     { "type": "object", "additionalProperties": { "type": "string" },
                   "default": { "PREFECT_API_URL": "http://prefect_server:4200/api",
                                "GIT_REPO": "<team-repo-url>" } },
      "networks":{ "type": "array",  "items": { "type": "string" }, "default": ["mlops"] },
      "volumes": { "type": "array",  "items": { "type": "string" },
                   "default": ["pipeline-data-cache:/cache", "pipeline-repo:/repo"] },
...
- `image` — flow 컨테이너로 쓸 Pipeline Flow 이미지 (§5.1). 태그 (`pipeline-flow:latest`) 가 곧 **런타임 버전** (라이브러리 + orchestrator) 입니다.
...
- `env` — flow 컨테이너가 server·Secret 을 찾는 `PREFECT_API_URL`, 그리고 팀 repo 주소 `GIT_REPO` (팀 공통 1개) 를 줍니다.
- `networks` — flow 컨테이너가 붙을 네트워크 (`mlops` 면 `minio`·`prefect_server` 를 서비스명으로 찾음).
- `volumes` — **이름 있는 공유 볼륨 두 개**. **`/repo`** (`pipeline-repo`) 에는 팀 repo 의 `.git` 이 누적되어 `git fetch` 가 증분으로 끝나고, **`/cache`** (`pipeline-data-cache`) 에는 데이터 버전 캐시가 보존됩니다. 둘 다 컨테이너 밖이라 일시적 컨테이너가 죽어도 살아남고, 버전 경로 (`v3_best` 등) 는 불변이라 공유해도 안전합니다.
- **볼륨 마운트는 Dockerfile 이 아닙니다** — 이미지는 "무엇이 들어 있는가" 만 정하고, `/repo`·`/cache` 같은 공유 볼륨은 **컨테이너를 띄우는 시점** 에 ST 는 이 base job template, LT 는 compose (Appendix C) 가 마운트합니다. 모두 같은 named volume 을 붙이므로 저장소가 공유됩니다.
...
flow 컨테이너에서 GPU 를 쓰려면 호스트에 NVIDIA 드라이버·nvidia-container-toolkit 이 있고 base job template 에서 GPU 를 요청해야 합니다. torch 의 CUDA 휠은 런타임을 번들하므로 호스트 드라이버가 최신이면 동작하며, 버전이 안 맞으면 베이스 이미지를 `nvidia/cuda` 계열로 교체합니다.
...
job 마다 뜨는 컨테이너의 python 환경입니다. **라이브러리와 orchestrator (`pipeline.py`) 만** 굽습니다. 팀 코드는 런타임에 공유 `/repo` 볼륨에서 `git fetch` + `git worktree` 로 원하는 커밋을 펼쳐 받습니다. 이미지가 한 번 빌드로 고정되어 모두 같은 런타임을 씁니다.
...
COPY pipeline.py .                    # orchestrator (deployment entrypoint); team repo is fetched at runtime into /repo
```

```powershell
# Build once at setup (libraries + orchestrator only). The team repo is fetched at runtime into the /repo volume.
docker build -t pipeline-flow:latest .
```

- **누가 실행하나** — `pipeline.py` 는 **deployment entrypoint** 라, dispatcher 가 띄운 Pipeline Flow 컨테이너 안의 Prefect 런타임이 `pipeline.py:pipeline` (@flow) 을 run 파라미터 (`git_commit`·`minio_version`·`entrypoint`) 와 함께 호출합니다. 사람이 직접 실행하지 않습니다.

### 5.2 Flow (Orchestrator)

orchestrator (flow) 는 **"커밋 받아 → 팀원 코드 실행"** 만 하는 얇은 골격이라 이미지에 굽습니다. 팀원의 실제 코드는 `git_commit` 으로 매 job 받아 와 **무슨 코드든 그대로 실행**됩니다 (`entrypoint` 로 스크립트 지정).

이 orchestrator (`pipeline.py`) 는 **플랫폼·관리자가 관리하는 공유·고정 골격** 이라 이미지에 포함되며 (deployment entrypoint), 팀원이 작성하지 않습니다. 팀원은 payload 스크립트 (`train.py` 등) 만 작성해 `entrypoint` 파라미터로 고릅니다.
...
REPO = "/repo"                         # shared named volume holding the team repo's .git
GIT_REPO = os.environ["GIT_REPO"]      # team-common repo URL (from the base job template env)

@flow(name="pipeline")
def pipeline(git_commit: str, minio_version: str, entrypoint: str = "train.py"):
    if not os.path.isdir(f"{REPO}/.git"):                                       # one-time bootstrap (no clone, no download)
        subprocess.run(["git", "init", REPO], check=True)
        subprocess.run(["git", "-C", REPO, "remote", "add", "origin", GIT_REPO], check=True)
        subprocess.run(["git", "-C", REPO, "config", "gc.auto", "0"], check=True)   # avoid concurrent auto-gc on the shared repo
    subprocess.run(["git", "-C", REPO, "fetch", "origin"], check=True)          # incremental; objects persist in /repo
    work = f"/tmp/run-{git_commit}"                                             # isolated working tree (unique path per run)
    subprocess.run(["git", "-C", REPO, "worktree", "add", "--detach", work, git_commit], check=True)
    ...
    subprocess.run(["git", "-C", REPO, "worktree", "remove", "--force", work], check=True)  # LT: prevent buildup; ST is auto-removed
```
...
> flow 컨테이너는 base job template 의 `PREFECT_API_URL` 로 server 에 연결돼야 Secret 을 받습니다 (§4.1). `mlflow`·`prefect` DB 는 사용자 코드가 직접 접속하지 않으므로, 사용자 role 에는 `catalog`·`optuna` 권한만 있으면 됩니다.
...
# GIT_REPO — team-common repo URL. LT worker reads it here; ST sets it in the base job template env (§4.1).
GIT_REPO=CHANGE_ME
```
...
단일 머신·소규모에서는 매 flow 컨테이너를 띄우는 대신 worker **자기 프로세스** 로 실행하는 Long Term Container 가 더 단순합니다. **Short Term 과 같은 `pipeline-flow` 이미지를 worker 로 띄우고** ([§5.1](#51-image)), 팀 repo·데이터는 ST 와 같은 `/repo`·`/cache` 볼륨을 마운트해 씁니다 (라이브러리·orchestrator 는 이미지에 이미 있음). run 들이 프로세스 공간을 공유하므로 격리는 약합니다.
...
services:
  prefect_worker:
    ...
    env_file:
      - docker-compose.env          # CONTROL_NODE_HOST, GIT_REPO (team repo URL)
    command: >
    ...
    volumes:
      - pipeline-repo:/repo          # team repo .git (init + fetch at runtime, shared)
...
- **ST·LT 동일 이미지** — ST 는 `pipeline-flow` 이미지를 job 마다 띄우고, LT 는 같은 이미지를 worker 로 상시 띄웁니다. 라이브러리·orchestrator 가 이미지에 있고 팀 repo 는 `/repo` 볼륨에서 fetch+worktree 로 받으므로, worker 에 따로 설치·전달할 게 없습니다. 특히 LT 는 한 컨테이너에서 여러 run 이 동시에 도므로 run 마다 격리된 worktree 가 필수입니다.
