...
단일 머신·소규모 구성에는 [Appendix F](#appendix-f-long-term-container-work-pool) 의 Long Term Container work pool 또는 serve mode 가 더 단순합니다.
...
- **자유로운 코드** — `entrypoint` 파라미터로 팀원이 자기 스크립트를 지정하므로 payload 형식을 강요하지 않습니다 (데이터 읽기·저장은 팀원 코드가 알아서 하고, 데이터 버전은 `MINIO_VERSION` 환경변수로 받습니다).
...
> 필요한 자격증명 (`MINIO_*`·`catalog`·`optuna`) 은 §6 처럼 `Secret.load(...)` 로 받습니다. MLflow 는 git repo 안에서 돌면 git 커밋을 자동 태그하므로 모델 ↔ 코드가 연결됩니다 (§8).
...
단일 머신·소규모 구성에서는 컨테이너를 매 run 띄우는 대신 dispatcher **자신의 프로세스** 로 실행하는 Long Term Container 방식이 더 단순합니다. 이때는 dispatcher 환경에 라이브러리를 직접 설치해야 하고, 코드는 bind-mount 로 그 머신에 둡니다. run 들이 같은 프로세스 공간을 공유하므로 격리는 약합니다.
...
- Long Term Container 방식은 코드를 마운트된 디스크에서 읽으므로 컨테이너로 코드를 전달하는 단계가 없습니다. 대신 그 머신에 코드가 있어야 합니다.
