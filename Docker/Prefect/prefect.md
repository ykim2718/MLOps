...
### Work Pool Registration
  ...
  **Base job template** — pool 이 띄우는 모든 `pipeline_flow` 컨테이너의 공통 설정입니다. flow 컨테이너는 dispatcher 의 마운트·네트워크를 상속하지 않으므로 **`PREFECT_API_URL` 과 네트워크를 여기서 명시** 합니다. 등급별로 `docker-pool-template-high.json`·`docker-pool-template-low.json` 두 벌을 두며 (`job_configuration` 은 같고 `variables` 의 `mem_limit` default 만 등급별로 다릅니다 — 아래는 high 예시, low 는 표 참고), 위 server compose 가 이를 server 컨테이너에 마운트해 둡니다.
  ...
  ```yaml
  # docker-pool-template-high.json
  {
    "variables": {
      "type": "object",
      "properties": {
...
### 4.2 Deployment
  ...
  server 에 **deployment** (어떤 flow 를·어느 pool 에서·어떤 이미지로 실행할지 — [Appendix A](#appendix-a-terminology)) 를 등록합니다. **플랫폼·관리자가 1회** 하는 일이며 (팀원·등급 구분은 deployment 이름·pool), 팀원이 작성하는 학습 스크립트 (`train.py`) 와는 무관합니다.
  ...
  - `parameters.payload: train.py` — flow 파라미터 기본값입니다 (`git_repo`·`git_commit`·`minio_version`·`member` 는 trigger 때 줍니다).
...
### 4.3 pipeline.py
  ...
  orchestrator (`pipeline.py`) 는 **"커밋 받아 → 팀원 코드 실행"** 만 하는 얇은 python 골격 (`@flow` 함수) 으로, [§4.1](#41-image) 이미지에 구워집니다. **플랫폼·관리자가 관리하는 공유·고정 골격** 이라 팀원이 작성하지 않습니다 — 팀원은 자기 학습 스크립트 (`train.py` 등) 만 작성해 `payload` 파라미터로 지정합니다.

  [§4.2](#42-deployment) 의 deployment 가 entrypoint 를 **`pipeline.py:pipeline`** 로 가리킵니다. 이 문자열은 server 의 deployment 레코드 (`prefect` DB) 에 저장되고, dispatcher 가 띄운 컨테이너 안에서 Prefect 런타임이 이미지 작업 디렉터리 (`/work`, `Dockerfile.pipeline_flow` 가 `pipeline.py` 를 COPY 한 곳) 기준으로 `pipeline.py` 를 import 해 콜론 뒤 **`@flow` 함수 `pipeline`** 을 run 파라미터 (`git_repo`·`git_commit`·`minio_version`·`payload`) 와 함께 호출합니다. 그래서 deployment entrypoint 가 곧 이 `pipeline.py` 입니다.
  ...
  def pipeline(git_repo: str, git_commit: str, minio_version: str, member: str = "", payload: str = "my_flow.py"):
      repo = tempfile.mkdtemp(prefix="repo-")    # fetch target with full history (per run)
      work = tempfile.mkdtemp(prefix="run-")     # isolated worktree pinned to the commit
      try:
          ...
          subprocess.run(["git", "-C", repo, "worktree", "add", "--detach", work, git_commit], check=True)  # check the commit out into a private worktree
          env = {**os.environ, "MINIO_VERSION": minio_version, "GIT_COMMIT": git_commit, "MEMBER": member}  # passed to the payload: data version + run identity
          subprocess.run(["python", payload], cwd=work, env=env, check=True)                              # run the team's payload in the private worktree
      finally:
  ...
  - **자유로운 코드** — `payload` 로 팀원이 자기 스크립트를 지정하므로 코드를 정해진 틀에 맞출 필요가 없습니다. 데이터 읽기·저장은 팀원 코드가 직접 하고, 데이터 버전은 `pipeline.py` 가 `minio_version` 파라미터를 `MINIO_VERSION` 환경변수로 넣어 주며 팀원 코드는 `os.environ["MINIO_VERSION"]` 으로 읽습니다.
  - **데이터 이력** — `minio_version` 이 **flow 파라미터** 라서 Prefect 가 run 마다 입력값을 `prefect` DB 에 자동 저장합니다. 데이터셋 버전·lineage 는 팀원 코드 (또는 공유 헬퍼) 가 카탈로그에 등록합니다.
...
- **deployment** — flow 를 어떻게 실행할지 묶어 **server DB (`prefect`) 에 저장한 레코드** 입니다. 파일·dict 가 아니라 server 안의 영구 레코드이고, API·UI·`prefect deployment inspect` 에서 **JSON 으로** 보입니다.
  ...
  - **사용** — 코드를 다시 안 봐도 이름 `<flow>/<deployment>` 로 run 을 trigger 합니다 (`prefect deployment run "pipeline/pipelineflow-high" -p payload=train.py` · UI · 스케줄). 그러면 dispatcher 가 그 정의대로 `pipeline_flow` 컨테이너를 띄웁니다.
...
- `prefect work-pool create <name> --type docker --base-job-template <file> [--overwrite]` — `docker` work pool 을 server 에 등록합니다 (§2).
