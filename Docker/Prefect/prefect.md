...
이 yaml 을 띄우는 명령입니다 (머신마다 1회). `run_dispatcher.ps1` ([Appendix F](#appendix-f-run_dispatcherps1)) 의 `docker compose up` 으로 컨테이너가 떠, 그 `command` 인 `prefect worker start` 가 컨테이너 안에서 실행됩니다.

```powershell
.\run_dispatcher.ps1 -WorkPool <등급>
```
