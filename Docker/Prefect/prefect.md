...
- **단일 호스트** — 위 구성은 한 호스트에서 모든 컨테이너가 `mlops` 네트워크를 공유하는 것을 기본으로 합니다. 다른 머신에 dispatcher 를 더 붙이는 방법은 [Appendix C](#appendix-c-additional-dispatcher-attachment) 를 참고합니다.
...
- **pool 타입** — 실행 환경을 어떻게 만드느냐로 갈립니다.
  - **Long Term Container** (`process` 타입) — dispatcher **자신의 상시 컨테이너 안에서 subprocess** 로 실행합니다. run 들이 같은 컨테이너를 공유합니다 (단일/소규모: [Appendix D](#appendix-d-long-term-container-work-pool)).
...
worker 는 **두 방식**을 지원합니다. **Short Term Container** (`docker` work pool, 주력) 에서는 worker 가 **dispatcher** 로서 job 마다 Pipeline Flow 컨테이너를 띄웠다 정리하고, **Long Term Container** (`process` work pool) 에서는 worker 가 **자기 상시 컨테이너 안에서 flow 를 직접 실행**합니다 (별도 컨테이너 없음). 이 절은 주력인 **Short Term Container** 설정을 다루며, Long Term Container 설정은 [Appendix D](#appendix-d-long-term-container-work-pool) 를, work pool type 정의는 [Appendix A](#appendix-a-terminology) 를 참고합니다.

dispatcher (`prefect_worker`) 는 Short Term Container work pool 을 polling 하다가 job 마다 **Pipeline Flow 이미지로 컨테이너를 새로 띄워** 코드를 실행시키고, 끝나면 정리합니다. 준비물은 두 가지입니다 — ① run 컨테이너 공통 설정 (base job template), ② dispatcher compose. (Pipeline Flow 이미지 자체는 [§5](#5-pipeline-flow-container) 에서 빌드합니다.)
...
이 설정은 아래 `docker-pool-template.json` 파일에 담아, **Short Term Container work pool 을 만들 때 한 번** 등록합니다 (`prefect work-pool create --type docker --base-job-template docker-pool-template.json`, §4.2 의 dispatcher 가 이 명령을 실행). 등록하면 내용이 **서버의 work pool 설정에 저장** 되어 이후 모든 run 에 자동 적용되므로, 팀원의 deploy 코드에는 다시 적지 않아도 됩니다.
...
- `image` — run 컨테이너로 쓸 Pipeline Flow 이미지입니다 (§5.1).
...
> **보안 주의** — 도커 소켓 마운트는 dispatcher 에 호스트 도커 전체 제어권 (사실상 root 권한) 을 줍니다. 신뢰된 내부망·스터디 용도로 한정합니다. 더 강한 격리가 필요하면 Kubernetes work pool 을 고려합니다 ([Appendix E](#appendix-e-orchestrator-benchmarking)).
...
처리량을 늘리려면 `--limit` 을 키우거나, 여러 머신에서 dispatcher 를 더 띄워 같은 pool 에 붙입니다 ([Appendix C](#appendix-c-additional-dispatcher-attachment)).
...
이 한 이미지를 **Short Term 과 Long Term 양쪽에 그대로** 씁니다 — ST (docker pool) 는 job 마다 이 이미지로 컨테이너를 띄우고, LT (process pool) 는 이 이미지를 worker 로 상시 띄웁니다 ([Appendix D](#appendix-d-long-term-container-work-pool)). 그래서 라이브러리를 worker 와 이미지에 중복 설치할 필요가 없습니다.
...
- **저장 위치** — 빌드한 이미지는 빌드를 실행한 호스트의 **로컬 Docker 이미지 스토어**에 저장됩니다 (`docker images` 로 확인). 같은 호스트의 dispatcher 는 같은 데몬을 보므로 그대로 씁니다. 여러 머신에서 쓰려면 레지스트리에 push 후 pull 하거나 각 머신에서 빌드합니다 ([Appendix C](#appendix-c-additional-dispatcher-attachment)).
...
- **자유로운 코드** — `entrypoint` 파라미터로 팀원이 자기 스크립트를 지정하므로, 팀원 코드를 정해진 틀에 맞출 필요가 없습니다. 데이터 읽기·저장은 팀원 코드가 직접 하며, 데이터 버전은 `MINIO_VERSION` 환경변수로 받습니다. **데이터 이력**은 두 층입니다 — `minio_version` 이 **flow 파라미터** 라서 Prefect 가 flow run 마다 입력값을 `prefect` DB 에 자동 저장하고 (UI 의 Flow Run → Parameters 에서 확인), 데이터셋 버전·lineage 는 팀원 코드 (또는 공유 헬퍼) 가 **카탈로그에 등록**해 남깁니다.
- **이력 자동 저장** — `@flow` 진입 시 Prefect 가 run 의 상태·로그·파라미터 (`git_commit`·`minio_version` 포함) 를 `prefect` DB 에 자동 기록합니다 (대시보드 Flow Runs). 지표·모델은 팀원 코드가 MLflow 로 로깅하면 함께 남습니다 ([Appendix F](#appendix-f-prefect-task)).
...
> run 컨테이너는 base job template 의 `PREFECT_API_URL` 로 server 에 연결돼 있어야 Secret 을 받을 수 있습니다 (§4.1). `mlflow`·`prefect` DB 는 사용자 코드가 직접 접속하지 않으므로, 사용자 role 에는 `catalog`·`optuna` DB 권한만 있으면 됩니다.

### docker-compose.env

위 Prefect Secret 이 **run 코드용** 자격증명이라면, **server·dispatcher 가 쓰는 값** (backend DB URL·Control Node 주소) 은 yml 에 평문으로 두지 않고 `docker-compose.env` 한 파일에 모읍니다. 실제 값이 담긴 `docker-compose.env` 는 `.gitignore` 로 제외하고, 비밀값을 비운 `docker-compose.env_example` 만 커밋합니다.
...
- dispatcher 는 코드를 실행하지 않으므로 MinIO·카탈로그 자격증명이 필요 없습니다 — 그 값들은 위 Prefect Secret 으로 전달합니다.
...
- **serve mode** — `flow.serve()` 를 실행하는 프로세스가 **상시 떠서 flow run 요청을 받아 처리**하는 모습이, 웹 서버가 요청을 받아 응답하듯 flow 를 계속 **제공 (serve)** 하기 때문에 붙은 이름입니다. `.serve()` 는 "이 flow 를 실행 가능한 상태로 계속 서빙하라" 는 뜻입니다.
...
## Appendix C. Additional Dispatcher Attachment
...
## Appendix D. Long Term Container Work Pool

단일 머신·소규모 구성에서는 컨테이너를 매 run 띄우는 대신 worker **자신의 프로세스** 로 실행하는 Long Term Container 방식이 더 단순합니다. **Short Term 과 같은 `pipeline-flow` 이미지를 worker 로 그대로 띄우면** ([§5.1](#51-image-팀-공통-소스-빌드본)), 라이브러리·소스가 이미 들어 있어 추가 설치·마운트가 필요 없습니다. run 들이 같은 프로세스 공간을 공유하므로 격리는 약합니다.
