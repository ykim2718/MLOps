...
| `prefect_dispatcher` | — | server 의 work pool 을 polling 해 `pipeline_flow` 컨테이너를 실행합니다. |
...
단일 머신·소규모에서는 매 flow 컨테이너를 띄우는 대신 dispatcher **자기 프로세스** 로 실행하는 Long Term Container 가 더 단순합니다. **Short Term 과 같은 `pipeline-flow` 이미지를 dispatcher 로 띄우고** ([§5.1](#51-image)), 라이브러리·orchestrator 는 이미지에 이미 있습니다. 팀 repo 는 ST 와 똑같이 run 마다 `git clone` 으로, 데이터는 MinIO 에서 직접 read 합니다. run 들이 프로세스 공간을 공유하므로 격리는 약합니다.
