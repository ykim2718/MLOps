...
- **deployment = 등급별 등록** — deployment 하나는 pool 하나에 바인딩되므로, 같은 flow 를 등급마다 등록해 (`pipeline/high`·`pipeline/low`) job 을 보낼 등급을 고릅니다 ([§3.2](#32-container)).
...
2) **PREFECT DISPATCHER** — 작업 머신마다 1대 · 직접 빌드
   ...
   ├─ build_dispatcher.ps1           이미지 빌드 (1회)
...
기동은 **`run_dispatcher.ps1`** (dispatcher 런처; 코드는 [Appendix G](#appendix-g-run_dispatcherps1)) 으로 합니다 — `.\run_dispatcher.ps1 -WorkPool <등급>` 으로 머신마다 1회 실행합니다.
...
.\run_dispatcher.ps1 -WorkPool high_performance
...
dispatcher 이미지 (`prefect-dispatcher:latest`) 를 1회 빌드하는 스크립트입니다 ([§3.1](#31-image)). `Dockerfile.dispatcher` (python + prefect + prefect-docker) 로 빌드하며, 이후 dispatcher 컨테이너는 부팅 때 설치 없이 `prefect worker start` 만 합니다.
