...
dispatcher (`prefect_worker`) 는 docker work pool 을 polling 하다가 job 마다 **Pipeline Flow 이미지로 컨테이너를 새로 띄워** 코드를 실행시키고, 끝나면 정리합니다. 준비물은 두 가지입니다 — ① run 컨테이너 공통 설정 (base job template), ② dispatcher compose. (Pipeline Flow 이미지 자체는 [§4](#4-pipeline-flow-container) 에서 빌드합니다.)
...
이 설정은 아래 `docker-pool-template.json` 파일에 담아, **docker work pool 을 만들 때 한 번** 등록합니다 (`prefect work-pool create --type docker --base-job-template docker-pool-template.json`, §3.2 의 dispatcher 가 이 명령을 실행). 등록하면 내용이 **서버의 work pool 설정에 저장** 되어 이후 모든 run 에 자동 적용되므로, 팀원의 deploy 코드에는 다시 적지 않아도 됩니다.
...
Pipeline Flow 는 dispatcher 가 job 마다 띄우는 per-run 컨테이너입니다. 여기서는 그 컨테이너의 **이미지 (어떻게 빌드하나)** 와 그 안에서 도는 **flow (무엇을 실행하나)** 를 다룹니다.
...
Pipeline Flow 의 flow 는 **파라미터로 받은 커밋으로 소스를 전환한 뒤 실제 코드를 실행** 하는 얇은 오케스트레이터입니다. 오케스트레이터 자체는 이미지에 구운 버전으로 고정되고, 바뀌는 payload 코드 (예: `train.py`) 는 `git checkout` 후 **하위 프로세스로** 실행합니다. 각 Step A~D 는 §1 의 컨테이너 내부 흐름과 같습니다.
...
- **pool 타입** — 실행 환경을 어떻게 만드느냐로 갈립니다.
  - **Long Term Container** (`process` 타입) — dispatcher **자신의 상시 컨테이너 안에서 subprocess** 로 실행합니다. run 들이 같은 컨테이너를 공유합니다 (단일/소규모: [Appendix F](#appendix-f-process-work-pool)).
...
| Aspect | Serve Mode | Work Pool — Long Term Container (`process`) | Work Pool — Short Term Container (`docker`) |
|--------|------------|---------------------|--------------------|
...
오케스트레이터가 **Pipeline Flow 이미지 (§4.1, `pipeline-flow:latest`)** 에 구워져 있으므로 별도 git 소스 지정 없이 **그 이미지의 entrypoint** 로 등록합니다 (server·worker 이미지가 아니라 Pipeline Flow 이미지입니다). 팀원·코드베이스 구분은 deployment 를 따로 두지 않고 **`git_commit` 파라미터** 로 처리합니다.
...
- `prefect work-pool create <name> --type docker --base-job-template <file> [--overwrite]` — docker work pool 을 만듭니다 (§3.2).
