...
- `env` — run 컨테이너가 server·Secret 을 찾는 `PREFECT_API_URL`, 그리고 팀 repo 주소 `GIT_REPO` (팀 공통 1개) 를 줍니다.
...
job 마다 뜨는 컨테이너의 python 환경입니다. **라이브러리와 orchestrator (`pipeline.py`) 만** 굽고, **팀 repo 는 굽지 않습니다.** 팀 코드는 런타임에 공유 `/repo` 볼륨에서 `git fetch` + `git worktree` 로 원하는 커밋을 펼쳐 받습니다. 이미지가 한 번 빌드로 고정되어 모두 같은 런타임을 씁니다.
...
- **`/repo` 볼륨** — 팀 repo 의 `.git` 이 사는 **이름 있는 공유 볼륨** (`pipeline-repo`) 을 모든 컨테이너에 `/repo` 로 마운트합니다 (§4.1). 객체가 볼륨에 누적되어 `git fetch` 가 증분으로 끝납니다. repo URL 은 `GIT_REPO` env 로 한 번 지정합니다 (팀 공통 1개).
- **볼륨 마운트는 Dockerfile 이 아닙니다** — 이미지는 "무엇이 들어 있는가" 만 정하고, `/repo`·`/cache` 같은 공유 볼륨은 **컨테이너를 띄우는 시점** 에 ST 는 base job template (§4.1), LT 는 compose (Appendix C) 가 마운트합니다. 모두 같은 named volume 을 붙이므로 저장소가 공유됩니다.
- 이미지 태그 (`pipeline-flow:latest`) 가 곧 **런타임 버전** (라이브러리 + orchestrator) 입니다.
...
단일 머신·소규모에서는 매 run 컨테이너를 띄우는 대신 worker **자기 프로세스** 로 실행하는 Long Term Container 가 더 단순합니다. **Short Term 과 같은 `pipeline-flow` 이미지를 worker 로 띄우고** ([§5.1](#51-image)), 팀 repo·데이터는 ST 와 같은 `/repo`·`/cache` 볼륨을 마운트해 씁니다 (라이브러리·orchestrator 는 이미지에 이미 있음). run 들이 프로세스 공간을 공유하므로 격리는 약합니다.
