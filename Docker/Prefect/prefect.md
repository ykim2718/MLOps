...
### 3.1 Image
  ...
  다음은 docker build를 위한 dockerfile과 execution command 입니다.
  ...
  - `FROM python:3.11.15` — 베이스 이미지입니다.
...
### 3.2 Container
  ...
  **머신마다 실행** — 같은 compose 를 각 컴퓨터에서 자기 등급 `WORK_POOL` 로 띄웁니다. pool 이 server 에 이미 있으니 (§2) dispatcher 는 polling 만 하며, 등급별 첫 머신/추가 머신 구분이 없습니다.
  ...
  `run_dispatcher.ps1` ([Appendix F](#appendix-f-run_dispatcherps1)) 은 yaml 을 띄우는 명령입니다 (머신마다 1회). 명령의 요소는 다음과 같습니다.
...
### 4.1 Image
  ...
  - `COPY requirements.txt` → `pip install` — 팀 라이브러리를 설치합니다 (코드보다 먼저 복사해 레이어 캐시를 살립니다).
...
### 4.2 Deployment
  ...
  다음은 deployment 정의 yaml 과 execution command 입니다.
...
### 4.3 pipeline.py
  ...
  def pipeline(git_repo: str, git_commit: str, minio_key: str, minio_bucket: str = "datasets",
      ...
      try:
          ...
          s3 = boto3.client("s3", endpoint_url=os.environ["MINIO_ENDPOINT"],                             # credentials from Prefect Secret (§5)
          ...
          subprocess.run(["python", payload,                                                              # run the team's payload in the worktree
                          ...
                          "--member", member, "--data", data], cwd=src, check=True)                      # --data points at the downloaded files
      finally:
...
### 4.4 Prefect UI
  ...
  - **Flow Runs** — trigger 된 run 이 `flow_run_name` 으로 나열됩니다. `member` 가 들어가 같은 deployment 아래에서 `alice@a1b2c3d` 처럼 **누구의 run 인지** 구분됩니다 ([§4.3](#43-pipelinepy) 의 `flow_run_name`). `pipeline.py` 가 `MEMBER`·`GIT_COMMIT` 을 payload env 로 넘기므로 팀 flow run 도 같은 이름을 씁니다.
  ...
  - **Parameters · State · Logs** — run 마다 입력 파라미터 (`git_repo`·`git_commit`·`minio_version`·`member`)·상태·로그가 자동 기록되어 (UI 의 Flow Run → Parameters), 같은 파라미터로 재실행 (재현) 할 수 있습니다.
  ...
  Flow Runs
    ...
    train      alice@a1b2c3d   Completed                        # team payload (@task), separate run
      ├─ data_prep      Completed
  ...
  같은 job 이 **flow run 두 개** 로 보입니다 — orchestrator (`pipeline`) 와 팀 payload (`train`). 둘 다 `flow_run_name` 이 `member@commit` 이라 묶어 보기 좋고, 팀 run 아래에 네 단계 task 가 달립니다. 팀 payload 가 plain 스크립트면 `train` run·task 없이 orchestrator run 만 보입니다.
