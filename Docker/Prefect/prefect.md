...
1) **PREFECT SERVER** — 제어 노드 1대 · 공식 이미지라 빌드 없음
   ...
   ├─ set_server.ps1                 기동: 네트워크 만들고 compose up
   ├─ set_pool.ps1                   work pool 등록 (서버 뜬 뒤 1회)
   ...
   ```powershell
   .\set_server.ps1 -ProjectName mlops -Yaml docker-compose.server.yml -Network mlops
   .\set_pool.ps1 -PoolName high_performance  -TemplateFile high.json -ConcurrencyLimit 16 -ProjectName mlops -Compose docker-compose.server.yml
   .\set_pool.ps1 -PoolName lower_performance -TemplateFile low.json  -ConcurrencyLimit 4  -ProjectName mlops -Compose docker-compose.server.yml
   ```
...
2) **PREFECT DISPATCHER** — 작업 머신마다 1대 · 직접 빌드
   ...
   └─ set_dispatcher.ps1             기동: compose up
   ```

   실행:

   ```powershell
   .\set_dispatcher.ps1 -WorkPool high_performance -WorkerLimit 8 -ProjectName mlops
   ```
...
3) **PIPELINE FLOW** — job 마다 떴다 사라지는 컨테이너 · 직접 빌드
   ...
   └─ set_flow.ps1                   이미지 빌드 (1회)
   ```

   실행:

   ```powershell
   .\set_flow.ps1 -Image pipeline-flow:latest
   ```
...
기동은 **`set_server.ps1`** (코드는 [Appendix D](#appendix-d-set_serverps1)) 으로 합니다.
...
**등록 (`set_pool.ps1`)** — server 안 prefect CLI 로 pool 마다 등록합니다 (`<Pool Name>`·`<Template File>`·`<Project Name>` 변수화; 코드는 [Appendix E](#appendix-e-set_poolps1)).
...
.\set_dispatcher.ps1 -WorkPool high_performance
...
## Appendix D. set_server.ps1

제어 노드에서 Prefect server compose 스택을 띄우는 기동 스크립트입니다 ([§2 Server Setup](#server-setup)). 공유 `mlops` 네트워크가 없으면 만들고 `docker-compose.server.yml` 을 올립니다. work pool 등록은 별도입니다 (`set_pool.ps1` — [Appendix E](#appendix-e-set_poolps1)).
...
```powershell
# set_flow.ps1 — (once) build the Pipeline Flow image (orchestrator + team libraries).
param(
