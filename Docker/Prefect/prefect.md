...
Prefect stack 을 한 호스트 서버 위에서 **세 구성요소 (Prefect Server · Prefect Worker · Pipeline Flow)** 로 나눠 도커로 실행하는 방법을 설명합니다. 실행 방식은 **Short Term Container** (job 마다 일시적 컨테이너를 띄웠다 파괴 — ephemeral docker-container-per-job) 와 **Long Term Container** (상시 컨테이너를 재사용) 두 가지를 지원합니다. **여러 팀원이 여러 코드베이스를 개발하고 동시에 다수 job 을 trigger** 하는 환경을 전제로 이 문서는 격리·동시성에 유리한 **Short Term Container 를 주력**으로 하며, 이를 Prefect 의 **Docker work pool** 로 실현합니다. job 마다 깨끗한 컨테이너가 떠서 코드를 실행하고 끝나면 스스로 사라지므로, run 들이 서로 간섭하지 않고 동시성 관리가 단순해집니다.
...
| **2. Prefect Worker** | dispatcher / executor | **Short Term Container** (주력) 에서는 work pool 을 polling 해 job 마다 Pipeline Flow 컨테이너를 띄우는 dispatcher 입니다 (코드 실행 안 함). **Long Term Container** 에서는 자기 컨테이너 안에서 flow 를 직접 실행합니다. | 상시 |
| **3. Pipeline Flow** | execution unit | flow (코드) 가 실제로 실행되는 곳입니다. 받은 커밋으로 git checkout 후 실행합니다 — **Short Term Container** (주력) 에서는 job 마다 뜨는 전용 ephemeral 컨테이너, **Long Term Container** 에서는 Prefect Worker 컨테이너 안에서 실행됩니다 (별도 컨테이너 없음). | Short Term: 일시적 (job 1개당 1개) / Long Term: worker 와 동일 (상시) |
...
| `prefect_worker` | — | Short Term Container work pool 을 polling 해 job 마다 Pipeline Flow 컨테이너를 띄우는 dispatcher 입니다. Long Term Container (`process`) 방식에서는 별도 컨테이너 없이 worker 자신의 상시 컨테이너 안에서 flow 를 직접 실행합니다. |
...
## 3. Prefect Worker Container

worker 는 **두 방식**을 지원합니다. **Short Term Container** (`docker` work pool, 주력) 에서는 worker 가 **dispatcher** 로서 job 마다 Pipeline Flow 컨테이너를 띄웠다 정리하고, **Long Term Container** (`process` work pool) 에서는 worker 가 **자기 상시 컨테이너 안에서 flow 를 직접 실행**합니다 (별도 컨테이너 없음). 이 절은 주력인 **Short Term Container** 설정을 다루며, Long Term Container 설정은 [Appendix F](#appendix-f-long-term-container-work-pool) 를, work pool type 정의는 [Appendix A](#appendix-a-terminology) 를 참고합니다.

dispatcher (`prefect_worker`) 는 Short Term Container work pool 을 polling 하다가 job 마다 **Pipeline Flow 이미지로 컨테이너를 새로 띄워** 코드를 실행시키고, 끝나면 정리합니다. 준비물은 두 가지입니다 — ① run 컨테이너 공통 설정 (base job template), ② dispatcher compose. (Pipeline Flow 이미지 자체는 [§4](#4-pipeline-flow-container) 에서 빌드합니다.)

### 3.1 Base Job Template
...
이 설정은 아래 `docker-pool-template.json` 파일에 담아, **Short Term Container work pool 을 만들 때 한 번** 등록합니다 (`prefect work-pool create --type docker --base-job-template docker-pool-template.json`, §3.2 의 dispatcher 가 이 명령을 실행). 등록하면 내용이 **서버의 work pool 설정에 저장** 되어 이후 모든 run 에 자동 적용되므로, 팀원의 deploy 코드에는 다시 적지 않아도 됩니다.
...
- `image` — run 컨테이너로 쓸 Pipeline Flow 이미지입니다 (§4.1).
...
- `command` — `prefect-docker` 설치 → `PREFECT_API_URL` 설정 → Short Term Container work pool 생성 (base job template 적용) → `prefect worker start` 순입니다. `CREATE_POOL`·`WORK_POOL`·`WORKER_LIMIT` 는 `docker compose up` 시점에 셸에서 읽는 compose 변수입니다 (미설정 시 `true`·`docker-pool`·`8`).
...
Short Term Container 방식은 run 마다 **별도 컨테이너** 라 동시 실행이 자연히 격리·병렬화됩니다. 동시 실행량은 세 가지로 조절합니다.
...
Pipeline Flow 는 dispatcher 가 job 마다 띄우는 per-run 컨테이너입니다. 여기서는 그 컨테이너의 **이미지 (어떻게 빌드하나)** 와 그 안에서 도는 **orchestrator flow (실행 골격)** 를 다룹니다.

각 Pipeline Flow 컨테이너는 **동일한 이미지 (팀 공통 소스 빌드본)** 에서 떠서, 받은 파라미터대로 내부에서 다음 단계를 수행합니다 (Step A·C 는 orchestrator 가, Step B·D 는 팀원 코드가 수행합니다).
...
- 이미지 태그 (`pipeline-flow:latest`) 가 곧 **런타임 버전** (라이브러리 + 베이스 소스) 입니다 ([§8](#8-code-delivery--versioning)).
- **저장 위치** — 빌드한 이미지는 빌드를 실행한 호스트의 **로컬 Docker 이미지 스토어**에 저장됩니다 (`docker images` 로 확인). 같은 호스트의 dispatcher 는 같은 데몬을 보므로 그대로 씁니다. 여러 머신에서 쓰려면 레지스트리에 push 후 pull 하거나 각 머신에서 빌드합니다 ([Appendix D](#appendix-d-additional-dispatcher-attachment)).
- **ST·LT 공용** — 이 한 이미지를 ST 와 LT 양쪽에 그대로 씁니다. ST (docker pool) 는 job 마다 이 이미지로 컨테이너를 띄우고, LT (process pool) 는 이 이미지를 worker 로 상시 띄웁니다 ([Appendix F](#appendix-f-long-term-container-work-pool)). 그래서 라이브러리를 worker 와 이미지에 **중복 설치할 필요가 없습니다.**

### 4.2 Flow (Orchestrator)

이미지에 굽는 orchestrator (flow) 는 **"커밋 받아서 → 팀원 코드 실행"** 만 하는 얇은 골격입니다. 팀원의 실제 코드는 굽지 않고 `git_commit` 으로 매 job 받아 와 **무슨 코드든 그대로 실행**됩니다 (`entrypoint` 로 스크립트 지정). 즉 이미지에 고정되는 건 골격뿐이고, 팀원 코드는 git 으로 동적 전달됩니다.
...
def pipeline(git_commit: str, minio_version: str, entrypoint: str = "train.py"):
    ...
    env = {**os.environ, "MINIO_VERSION": minio_version}                   # pass the data version to the team's code
    subprocess.run(["python", entrypoint], cwd=REPO, env=env, check=True)  # run the team's code as-is
```

- **자유로운 코드** — `entrypoint` 파라미터로 팀원이 자기 스크립트를 지정하므로 payload 형식을 강요하지 않습니다. 데이터 읽기·저장은 팀원 코드가 직접 하며, 데이터 버전은 `MINIO_VERSION` 환경변수로 받습니다. **데이터 이력**은 두 층입니다 — 어떤 버전을 썼는지 (`minio_version`) 는 Prefect 가 run 에 **자동 기록**하고, 데이터셋 버전·lineage 는 팀원 코드 (또는 공유 헬퍼) 가 **카탈로그에 등록**해 남깁니다.
- **이력 자동 저장** — `@flow` 진입 시 Prefect 가 run 의 상태·로그·파라미터 (`git_commit`·`minio_version` 포함) 를 `prefect` DB 에 자동 기록합니다 (대시보드 Flow Runs). 지표·모델은 팀원 코드가 MLflow 로 로깅하면 함께 남습니다 ([§8](#8-code-delivery--versioning), [Appendix H](#appendix-h-prefect-task)).
...
> run 컨테이너는 base job template 의 `PREFECT_API_URL` 로 server 에 연결돼 있어야 Secret 을 받을 수 있습니다 (§3.1). `mlflow`·`prefect` DB 는 사용자 코드가 직접 접속하지 않으므로, 사용자 role 에는 `catalog`·`optuna` DB 권한만 있으면 됩니다.
...
- **이름의 유래**: 그 프로세스가 **상시 떠서 flow run 요청을 받아 처리**하는 모습이, 웹 서버가 요청을 받아 응답하듯 flow 를 계속 **제공 (serve)** 하기 때문입니다 — `.serve()` 는 "이 flow 를 실행 가능한 상태로 계속 서빙하라" 는 뜻입니다.
- **실행 위치·환경**: `.serve()` 를 띄운 그 머신 (보통 팀원 client) 의 python 환경에서 실행되므로, 별도 이미지·제약 없이 팀원이 자유롭게 환경을 씁니다 (work pool mode 는 반대로 server 측 공용 이미지로 실행을 통일합니다).
...
- **pool 타입** — 실행 환경을 어떻게 만드느냐로 갈립니다.
  - **Long Term Container** (`process` 타입) — dispatcher **자신의 상시 컨테이너 안에서 subprocess** 로 실행합니다. run 들이 같은 컨테이너를 공유합니다 (단일/소규모: [Appendix F](#appendix-f-long-term-container-work-pool)).
  - **Short Term Container** (`docker` 타입, 주력) — job 마다 **새 컨테이너 (Pipeline Flow) 를 띄워 실행하고 끝나면 파괴하는 ephemeral container** 입니다. run 마다 격리되고, 이미지를 통해 런타임을 통일할 수 있어 여러 팀원·동시 실행에 적합합니다.
...
git 으로 전달되어 컨테이너 안에서 `python train.py` 로 실행되는 **실제 ML 코드** 예시입니다. 오케스트레이터 (§4.2) 가 이 코드를 하위 프로세스로 부릅니다 — 바뀌는 부분은 이 payload 이고, `git_commit` 으로 어떤 버전을 돌릴지 고릅니다.
...
> 필요한 자격증명 (`MINIO_*`·`catalog`·`optuna`) 은 §5 처럼 `Secret.load(...)` 로 받습니다. MLflow 는 git repo 안에서 돌면 git 커밋을 자동 태그하므로 모델 ↔ 코드가 연결됩니다 (§8).

### Deployment & Trigger

**Pipeline Flow 이미지 (§4.1, `pipeline-flow:latest`)** 에 코드가 구워져 있으므로 별도 git 소스 지정 없이 **그 이미지의 entrypoint** 로 등록합니다 (server·worker 이미지가 아니라 Pipeline Flow 이미지입니다). 팀원·코드베이스 구분은 deployment 를 따로 두지 않고 **`git_commit` 파라미터** 로 처리합니다.
...
- **Short Term Container (ephemeral container)** — `docker` work pool 이 job 마다 새로 띄웠다 끝나면 파괴하는 일시적 실행 컨테이너입니다. 이 문서의 Pipeline Flow 가 여기 해당합니다.
- **Long Term Container** — `process` work pool 에서 여러 job 을 자기 안 subprocess 로 실행하는, 공유되는 상시 컨테이너입니다 (worker 컨테이너 자체).
- **work pool** — job 이 대기하는 큐이자 실행 방식 (type) 의 정의입니다. server 안의 메타데이터이며 컨테이너가 아닙니다.
- **work pool type** — Prefect 가 정한 work pool 의 실행 방식 이름입니다. `process` (Long Term Container 안 subprocess) · `docker` (job 마다 Short Term Container) · `kubernetes` (job 마다 pod) · `ecs` 등이 있고, 타입이 "job 을 어떤 실행 단위로 돌리나" 를 정합니다.
...
- `prefect config set PREFECT_API_URL="http://<Host IP>:4200/api"` — client 가 바라볼 server 주소를 프로필에 1회 저장합니다 (§7).
- `prefect server start --host 0.0.0.0` — Prefect server 를 기동합니다 (§2).
- `prefect work-pool create <name> --type docker --base-job-template <file> [--overwrite]` — Short Term Container work pool 을 만듭니다 (§3.2).
- `prefect work-pool get-default-base-job-template --type docker` — 도커 worker 의 기본 base job template 을 출력합니다 (§3.1).
...
- `prefect deploy` (또는 `flow.deploy(...)`) — deployment 를 등록합니다 (§7).
- `prefect deployment run "<flow>/<deployment>" -p <key>=<value>` — 등록된 deployment 를 파라미터와 함께 trigger 합니다 (§7).
...
- dispatcher 는 코드를 실행하지 않으므로 MinIO·카탈로그 자격증명이 필요 없습니다 — 그 값들은 Pipeline Flow 컨테이너의 코드가 Prefect Secret 블록으로 받아 씁니다 (§5 Credentials).
...
dispatcher 는 server 와 분리돼 있어 그 머신에서 Short Term Container work pool 을 polling 하기만 하면 합류합니다. 전제로 그 컴퓨터에서 server (`http://<Host IP>:4200`) 에 네트워크로 접근 가능해야 하고, Pipeline Flow 이미지가 있어야 합니다.
...
## Appendix F. Long Term Container Work Pool

단일 머신·소규모 구성에서는 컨테이너를 매 run 띄우는 대신 worker **자신의 프로세스** 로 실행하는 Long Term Container 방식이 더 단순합니다. **Short Term 과 같은 `pipeline-flow` 이미지를 worker 로 그대로 띄우면** ([§4.1](#41-image-팀-공통-소스-빌드본)), 라이브러리·소스가 이미 들어 있어 추가 설치·마운트가 필요 없습니다. run 들이 같은 프로세스 공간을 공유하므로 격리는 약합니다.

```yaml
# docker-compose.worker.yml (process variant — single/small scale)
services:
  prefect_worker:
    image: pipeline-flow:latest     # the same image as the Short Term run container (§4.1)
    env_file:
      - docker-compose.env
    command: >
...
- **ST·LT 동일 이미지** — ST (docker pool) 는 `pipeline-flow` 이미지를 job 마다 띄우고, LT (process pool) 는 같은 이미지를 worker 로 상시 띄웁니다. 라이브러리·소스가 이미지에 들어 있어, 코드는 git checkout 으로 전환할 뿐 별도 전달·설치 단계가 없습니다 (worker 에 라이브러리를 따로 설치하지 않으므로 중복이 없습니다).

## Appendix G. Orchestrator Benchmarking

"**가벼운 에이전트 (dispatcher) 가 작업을 집어, 작업마다 격리된 일시적 실행 단위를 띄워 실행하고 정리**" 하는 패턴은 오케스트레이션 전반의 업계 표준입니다. 이 스택의 Short Term Container work pool 은 그 표준의 **단일 호스트 변형** 이고, 규모가 커지면 실행 단위를 컨테이너 → **pod** 로 올린 Kubernetes 변형으로 자연스럽게 확장됩니다.
...
- **pod** — Kubernetes 의 **최소 실행/배포 단위** 입니다. 컨테이너 하나 이상이 **같은 네트워크 (IP)·스토리지를 공유** 하며 한 덩어리로 스케줄됩니다. 오케스트레이션에서 "작업 1개 → pod 1개" 가 격리 단위가 됩니다. 즉 Short Term Container 자리에 클러스터 규모에서 들어가는 것이 pod 입니다 (Prefect/Airflow 의 개념이 아니라, 그 아래 Kubernetes 의 실행 껍데기입니다).
...
`@task` 를 쓰지 않아도 이력 관리와 재현 (reproducibility) 은 완벽하게 됩니다. Prefect 에서 전체 실행 흐름을 묶는 핵심 단위는 `@task` 가 아니라 **`@flow`** 이기 때문입니다. 파이썬 코드 위에 `@flow` 데코레이터만 붙이면, 그 안에서 도는 코드가 일반 함수든 클래스든 상관없이 **실행 이력과 입력 파라미터가 Prefect Server 에 기록**됩니다.

### Reproducing without @task

`@task` 를 전혀 쓰지 않고 `@flow` 와 일반 파이썬 함수만으로 과거 시점 (git 커밋 + MinIO 데이터 버전) 을 재현하는 구조입니다.
...
이렇게 짜도 이력·재현이 되는 이유는 둘입니다.

- **파라미터 추적** — Prefect Server 는 `@flow` 진입 시 들어온 인자 (`git_commit_hash`·`minio_data_version`) 를 DB 에 기록합니다. 나중에 UI 에서 그 기록을 보고 같은 파라미터로 재실행 (재현) 할 수 있습니다.
- **상태 관리** — flow 전체의 성공 (Completed) / 실패 (Failed) 여부와 로그가 기록되므로 이력 관리에 문제가 없습니다.

### Why use @task then

`@task` 없이도 이력은 남지만, 쓰는 이유는 **실패 복구**와 **성능** 입니다. 안 쓰면 다음을 잃습니다.
...
이력 관리와 과거 재현은 **`@flow` 에 파라미터 (git 커밋·MinIO 버전) 를 넘기는 것만으로 완전히 작동**합니다. 학습 소스가 클래스 덩어리라 `@task` 를 일일이 붙이기 번거롭다면, `@task` 를 생략하고 큰 틀에 `@flow` 만 씌워도 MLOps 재현 목적에는 지장이 없습니다.
