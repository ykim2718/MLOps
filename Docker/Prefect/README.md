...
이 워크플로우는 여러 팀원이 한 서버를 공유하며 AI 학습을 돌릴 때, **데이터·실험·결과를 잃어버리지 않고 추적·재현·공유**하기 위한 다음 여덟 가지를 핵심 목표로 합니다.

1. **End-to-end lineage — 양방향 (계보 추적)**
   데이터·코드·결과를 양방향으로 역추적합니다.<br>
   정방향: 데이터 + 코드 $\rightarrow$ 결과 (생성 과정 추적)<br>
   역방향: 결과 $\rightarrow$ 데이터 + 코드 (원인 분석 및 버전 확인)

2. **Monitoring (모니터링)**
   **Prefect / MLflow / MinIO** 각 대시보드로 동시에 도는 작업과 결과 현황을 한눈에 관리합니다.

3. **Reproducibility (재현성)**
   **데이터 버전·모델 아키텍처·하이퍼파라미터·난수 시드(seed)** 등을 **고정**하여 **동일 결과를 보장**합니다.

4. **Reusability (재사용성)**
   한 번 만든 **워크플로우나 피처(feature)** 를 **다른 프로젝트에서도 다시 쓸 수 있게 자산화**합니다.

5. **Persistence (지속성/영속성)**
   학습된 **모델이나 파이프라인의 상태**를 증발시키지 않고 **디스크/스토리지에 안전하게 저장·유지**합니다.

6. **Resource Management (자원 관리)**
   GPU/CPU 등 **공유 자원**을 work pool과 동시 실행 수 제한(`--limit`)으로 **분배·관리**해, 충돌 없이 여러 팀원의 잡을 한 서버에서 돌립니다.

7. **Scheduled Automation (예약형 자동화)**
   cron/interval 스케줄로 **야간/휴일에도 공백 없는 순차적 무인 작업 실행**이 가능합니다.

8. **Data version management (데이터 버전 관리)**
   데이터셋의 메타데이터를 저장하여 **Data Catalog**를 제공하고, **Selective Retrieval (선택적 다운로드)**가 가능하게 합니다. (메타데이터는 PostgreSQL `catalog` DB, 실제 데이터는 MinIO)

### Stack

스택의 서비스는 docker로 실행됩니다 (**[Appendix A. Docker Setup](#a-docker-setup)** 참고).

| Component      | Service             | Role                                                                                                                                                     | Dashboard                                                    | Note                                                                | Chapter            |
|----------------|---------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------|---------------------------------------------------------------------|--------------------|
| **Prefect**    | `server` · `worker` | 오케스트레이션(파이프라인 실행/스케줄링).<br>server=서버+대시보드(UI), worker=잡 실행.<br>목적: 모니터링, 계보(코드 축), 재사용성(deployment),<br>자원 관리(work pool·--limit), 예약형 자동화(cron/interval) | Prefect UI<br>**http://localhost:4200**                      | `worker default --limit 8`                                          | [6](#6-prefect)    |
| **MinIO**      | `minio`             | 대용량 데이터/모델/아티팩트 저장(S3 호환).<br>목적: 데이터 검색·선택·다운로드, 데이터 버전 관리,<br>지속성(모델·데이터 영속 저장)                                                                        | MinIO Console<br>**http://localhost:9001**<br>(S3 API :9000) | 3 buckets:<br>`datasets`,<br>`models`,<br>`mlflow`                  | [3](#3-minio)      |
| **MLflow**     | `mlflow`            | 실험(params·metrics) 추적, 모델 레지스트리.<br>목적: 계보(결과 축), 재현성(params·seed 기록)                                                                                    | MLflow UI<br>**http://localhost:5000**                       | backend=`postgres`,<br>artifact=`minio`                             | [4](#4-mlflow)     |
| **PostgreSQL** | `postgres`          | 모든 도구의 메타데이터 DB(4개 논리 DB).<br>목적: 데이터 검색·버전·계보 인덱스(catalog)                                                                                              | :5432                                                        | 4 logical DBs:<br>`prefect`,<br>`mlflow`,<br>`optuna`,<br>`catalog` | [2](#2-postgresql) |
| **Optuna**     | tuner               | 하이퍼파라미터 튜닝(trial 탐색).<br>목적: 재현성(하이퍼파라미터 고정/탐색)                                                                                                          | study storage는<br>`postgres`(`optuna` DB)                    | Python module                                                       | [5](#5-optuna)     |
...
[ Prefect Server + Worker  :4200 ]
   ├─ run stages ─────▶ [ MLflow Server  :5000 ]
   ├─ tuning ─────────▶ [ Optuna  study ]
   ├─ run state/logs ─▶ [ PostgreSQL  :5432 ]
   └─ data/models ────▶ [ MinIO ]

[ MLflow Server  :5000 ]
   ├─ params·metrics ─▶ [ PostgreSQL  :5432 ]
   └─ artifact ───────▶ [ MinIO ]

[ Optuna  study ]
   └─ trial results ──▶ [ PostgreSQL  :5432 ]
```
...
하나의 PostgreSQL 인스턴스 안에 **도구별 메타데이터 전용 DB 4개**를 둡니다. 대용량 바이너리는 절대 DB에 넣지 않고 MinIO에 둡니다([Chap. 3](#3-minio) 참고).

| DB        | Reader         | Writer | Role                  | Metadata                             | Data Site               |
|-----------|----------------|--------|-----------------------|--------------------------------------|-------------------------|
| `prefect` | Prefect UI     | Python | Prefect backend store | run 상태/스케줄/로그/파라미터 메타                | MinIO (경로 참조)           |
| `mlflow`  | MLflow UI      | Python | MLflow backend store  | params/metrics/tags,<br>run/registry | MinIO s3://mlflow/...   |
| `optuna`  | Optuna sampler | Python | Optuna study storage  | trial별 params·value·상태               | MinIO (체크포인트 경로)        |
| `catalog` | catalog.py     | Python | Data catalog          | 데이터셋 이름·버전·해시·스키마·계보                 | MinIO s3://datasets/... |

> 4개 DB는 `docker-compose.yml`의 init SQL이 컨테이너 최초 기동 시 자동 생성합니다([Appendix A. Docker Setup](#a-docker-setup) 참고).

### DB = 메타데이터, MinIO = 대용량 데이터

Prefect · MLflow · Optuna · (데이터) catalog는 공통적으로 **DB에는 "메타데이터"만 저장**합니다. 모델 가중치·데이터셋·plot 같은 **실제(대용량) 데이터는 DB에 넣지 말고**, **MinIO(오브젝트 스토리지, [Chap. 3](#3-minio) 참고)** 에 두고 **경로(URI)/버전ID만 DB가 참조**하도록 배치하는 것이 원칙입니다.

> 핵심 원칙: **DB = 작은 구조화 메타데이터, 대용량 바이너리 = MinIO + 경로 참조**
>
> 도구별 메타데이터(Metadata)와 실제 데이터 위치(Data Site)는 위 [Stack 표 다음의 DB 표](#2-postgresql)에 정리되어 있습니다.
...
- **버저닝(versioning)**: 같은 키로 다시 올려도 이전 오브젝트를 `version_id`로 보존하는 기능. 이 스택에서 데이터 버전 관리의 1차 수단은 "경로 분리(`/v1/`, `/v2/`) + catalog 레코드"이고, MinIO 버저닝은 **덮어쓰기 사고를 막는 보조 안전장치**입니다([Chap. 9](#2-data-versioning--이력-저장--이전-버전-보존--버전-선택) 참고).

### Installing mc

`mc`(MinIO Client)는 MinIO 서버와 **별개의 CLI 도구**라 따로 설치해야 명령이 인식됩니다. (설치 안 됐으면 실행 시 `command not found`가 납니다. Python과는 무관합니다.)

- **Windows 패키지 매니저**: `scoop install minio-client` 또는 `choco install minio-client` *(scoop·choco는 Windows용 앱 설치 도구이며 Python과 무관)*
- **바이너리 직접**: `https://dl.min.io/client/mc/release/windows-amd64/mc.exe` 를 받아 PATH 폴더에 두기
- **설치하기 싫으면**: 도커로 실행 — `docker run --rm minio/mc ...`(아래 예시). 버킷 생성·버저닝은 compose의 `createbuckets`가 자동 처리하므로 `mc` 없이도 됩니다.

### Creating Buckets + Enabling Versioning (`mc` CLI)

> ✅ `docker compose up` 을 쓰면 compose의 `createbuckets` 서비스가 버킷 생성 + 버저닝을 **자동 처리**합니다([Appendix A](#a-docker-setup) 참고). 따라서 아래 수동 `mc` 실행은 **선택사항**입니다(스택을 쓰지 않거나 직접 제어하고 싶을 때만). `mc` 설치는 위 [Installing mc](#installing-mc) 참고.
...
> ⚠️ MinIO **커뮤니티 콘솔(:9001)에는 버킷 생성·버저닝 관리 메뉴가 없습니다** (관리 기능이 상용 제품으로 분리됨). 따라서 위 `mc` CLI를 사용하세요. `mc`를 설치하지 않았다면 도커로 한 번에 실행할 수 있습니다(스택 컨테이너가 `docker compose up -d`로 실행 중일 때):
> ```powershell
> docker run --rm --network prefect_default minio/mc sh -c "mc alias set local http://minio:9000 minioadmin minioadmin && mc mb local/datasets local/models local/mlflow ; mc version enable local/datasets && mc version enable local/models"
> ```
...
> 커뮤니티 웹 콘솔에는 버저닝 표시가 없으므로 위 `mc`(또는 boto3)로 확인합니다.
...
> 요약: 개별 실험이면 best 5는 **MinIO로 충분**합니다. 배포·서빙까지 가면 그 모델을 **MLflow Model Registry**에 등록하면 됩니다([Chap. 4](#4-mlflow) 참고).

---

## 4. MLflow

**MLflow**는 실험의 **params·metrics를 추적**하고, 학습된 **모델을 레지스트리로 관리·배포·서빙**하는 도구입니다. 이 스택에서는 저장소를 두 곳으로 나눕니다.

- **Backend store** = PostgreSQL `mlflow` DB — params·metrics·tags·run/registry 메타데이터
- **Artifact store** = MinIO `s3://mlflow/...` — 모델·plot·파일 등 실제 산출물
...
> Prefect는 "실행 흐름", MLflow는 "실험 기록"을 담당해 역할이 겹치지 않습니다. 각 단계 task 안에서 `mlflow.log_*`로 같은 run에 기록하는 패턴은 [Chap. 7](#7-pipeline)를 참고하세요.

### Versioning, Deploying, and Serving the Best Model
...
**Optuna**는 하이퍼파라미터를 trial 단위로 탐색하는 튜닝 도구입니다. `objective`(목적 함수)를 Optuna가 매 trial마다 호출해 하이퍼파라미터를 제안받고 점수를 반환받으며, 그 점수를 보고 다음 trial을 더 똑똑하게 고릅니다.

```python
def objective(trial):
    lr = trial.suggest_float("lr", 1e-4, 1e-1, log=True)
    n_estimators = trial.suggest_int("n_estimators", 50, 300)
    # ... 위 파라미터로 학습/검증 후 점수 반환 ...
...
> 어느 쪽이든 Optuna가 DB에 넣는 것은 **trial 메타데이터(파라미터·점수)뿐**이고, 모델 가중치 같은 실제 산출물은 DB가 아니라 MinIO에 저장합니다([Chap. 3](#where-to-store-the-best-models-weights) 참고).
...
- **분산 실행(Worker / Work Pool)**: 워커와 work pool 구조로 잡을 분리 실행하고 **동시 실행 수를 제어**할 수 있어, 여러 팀원의 AI 학습 잡을 한 서버에서 관리하기에 적합합니다. (로컬 PC에서 워커를 붙이는 방법은 [Appendix C](#c-attaching-a-worker-to-a-work-pool-from-a-local-pc-선택) 참고)
...
같은 flow를 이름과 파라미터만 다르게 하여 여러 deployment로 등록해 두면, 팀원/실험별로 독립 실행할 수 있습니다. ([Chap. 8](#method-b--register-as-a-deployment-and-trigger-on-demand) 참고)
...
Prefect 클라이언트가 **어느 서버에 연결할지**, 그리고 산출물·메타데이터를 **어디에 배치할지(placement)** 에 대한 설정입니다. (도커로 서버를 띄우는 방법은 **[Appendix A. Docker Setup](#a-docker-setup)** 참고)
...
- **재현성**: DB의 메타데이터(어떤 데이터·하이퍼파라미터로 돌렸는지)와 MinIO의 실제 산출물(가중치·데이터셋)을 **경로(URI)/버전ID/해시로 연결**해 두면 실험 재현이 쉬워집니다. ([Chap. 9](#9-data) 참고)
...
각 팀원은 자기 학습 **코드와 설정만** 만들면 됩니다. 이 폴더 안에는 코드만 두고, 실행 결과물은 여기에 쌓이지 않습니다(서버의 PostgreSQL/MinIO로 들어갑니다). (실제 동작 예시는 `example/` 폴더와 **[Appendix D](#d-pipeline-orchestrator)** 참고)

```
team_member_1/                 # 팀원별 작업 폴더 (코드/설정만 둠)
├─ catalog.py                  # 카탈로그 접근 계층 (Appendix B)
└─ example/                    # 오케스트레이터 + 단계 스크립트 + 설정
   ├─ prefect_run.py           #   Prefect 오케스트레이터 (Appendix D)
...
`data preparation (dp) → feature engineering (fe) → training (train) → test` 순으로 진행할 때, 각 단계의 **산출물이 다음 단계의 입력으로 어떻게 연결**되는지를 정리합니다. 각 단계는 Prefect의 `@task`로 감싸고, `@flow`가 순서를 강제합니다(앞 단계 산출물이 있어야 다음 단계 실행). 실제 동작 코드는 `example/` 폴더와 **[Appendix D](#d-pipeline-orchestrator)**에 있습니다.
...
- **Prefect ↔ Optuna**: 튜닝을 각 단계의 task로 감싸 best 설정/하이퍼파라미터를 받아 다음 단계로 넘깁니다.
- **Prefect ↔ MLflow**: 각 단계 task 안에서 `mlflow.log_*`로 params·metrics·산출물을 같은 run에 기록합니다.

> 각 단계를 세 도구로 감싸는 **구체적인 코드 패턴**은 **[Appendix D. Pipeline Orchestrator](#d-pipeline-orchestrator)** 의 "완성형 패턴"을 참고하세요.

---

## 8. Execution

`example/prefect_run.py`(**[Appendix D](#d-pipeline-orchestrator)**)는 `prefect_configuration.json`을 읽어 `example/` 폴더의 단계들을 순서대로 실행하는 Prefect 오케스트레이터입니다. 설정 파일은 같은 폴더(`example/`)에 이미 준비되어 있습니다 ([prefect_configuration.json](example/prefect_configuration.json)).
...
서버 연결(`PREFECT_API_URL`)은 [Chap. 6](#prefect-configuration)에서 이미 설정했다고 가정합니다.
...
즉, `python example/prefect_run.py`를 실행하면 **바로 학습이 도는 게 아니라**, "member1-mnist-resnet50"이라는 deployment가 등록되고 프로세스가 **트리거(실행 요청)를 기다리는 상태**가 됩니다. 실제 실행은 아래처럼 별도로 트리거할 때 일어납니다.
...
`catalog`은 PostgreSQL `catalog` DB 안의 **테이블 하나(`datasets`)** 입니다. MinIO에 있는 실제 데이터를 가리키는 **메타데이터 장부**로, 데이터 자체는 MinIO에, 그 데이터에 대한 **"설명·이력·위치"** 는 catalog에 둡니다. 이 장부를 읽고 쓰는 헬퍼 모듈이 `catalog.py` 입니다(아래 (1)~(3) 코드에서 사용).

> 테이블 스키마(`datasets` DDL), `catalog.py` 헬퍼 코드, 그리고 **컬럼 보강(확장) 방법**은 **[Appendix B. Catalog Schema](#b-catalog-schema)** 에 정리되어 있습니다.
...
### (2) Data Versioning — 이력 저장 · 이전 버전 보존 · 버전 선택
...
> **이름 규칙 (`DatasetId` · `Version` 공통)**: **소문자(a-z)·숫자(0-9)·underscore(`_`)·점(`.`)만** 사용합니다. **공백·대문자·dash(`-`)·기타 특수문자는 불가**합니다(이 값이 그대로 MinIO 경로와 catalog 키가 되기 때문). 예: `v1`, `v2`, `v1.0`. `data_uploader.ps1`/`data_downloader.ps1`은 위반 시 **에러로 즉시 중단**됩니다.
>
> **MinIO 경로 네임스페이스**: `s3://datasets/<DatasetId>/<Version>/...`
> 예) `DatasetId=sydney_202605`, `Version=v2` → `s3://datasets/sydney_202605/v2/...`
> 또한 `data_uploader.ps1`은 업로드 전에 이 경로가 MinIO에 **이미 있는지(중복) 확인**하고, 중복이면 다른 버전을 재입력하거나 취소(중단)합니다.
...
catalog.register(
    ...
    created_by="zoo", prefect_run_id=flow_run.get_id(),
    metadata={"fab": "fab2", "chamber": "CH3", "step": 5, "parent_version": "v1"},
)
```

> (선택) MinIO 버킷 자체의 versioning(`mc version enable local/datasets`, [Chap. 3](#3-minio))을 함께 켜두면, 실수로 같은 키를 덮어써도 오브젝트 단위로 한 번 더 보호됩니다. 다만 **버전 관리의 1차 수단은 "경로 분리 + catalog 레코드"** 입니다.
...
> best 모델을 버전 관리·배포·서빙하는 방법은 [Chap. 4](#versioning-deploying-and-serving-the-best-model) 참고.
...
> 데이터 카탈로그 **테이블(`datasets`)** 은 여기(인프라)서 만들지 않고 **코드로 멱등 생성**합니다 — [Appendix B. Catalog Schema](#b-catalog-schema)의 `ensure_schema()` 참고.
...
- **팀원이 각자 실행해야 하는 경우**: 각 팀원의 **로컬 GPU/자원**에서 학습을 돌려야 한다면, 팀원이 자기 PC에서 같은 work pool(`default`)에 워커를 붙이면 됩니다. (붙이는 방법은 **[Appendix C](#c-attaching-a-worker-to-a-work-pool-from-a-local-pc-선택)** 참고)

### B. Catalog Schema

`catalog`은 PostgreSQL `catalog` DB 안의 **테이블 하나(`datasets`)** 이고, 그 장부를 읽고 쓰는 헬퍼 모듈이 **[`catalog.py`](catalog.py)** 입니다. ([Chap. 9](#9-data) 에서 사용)

#### `catalog.py` 는 무엇이고, 워크플로우에서 무엇을 담당하나

`catalog.py` 는 **데이터 카탈로그(메타데이터 장부)를 다루는 작은 라이브러리**입니다. 실제 데이터 파일은 옮기지 않고(그건 boto3/MinIO 담당), **"무엇이 · 어디에 · 어느 버전으로 · 누가 만들었는지"를 `catalog` DB에 기록·조회**하는 일만 합니다. 즉 **Prefect 태스크 ↔ PostgreSQL `catalog` DB** 사이에 놓인 **카탈로그 접근 계층**입니다.

워크플로우의 4대 목적([Chap. 1](#1-workflow)) 중 이것을 담당합니다:

| 함수 | 담당 목적 | 하는 일 |
|------|-----------|---------|
| `ensure_schema()` | (공통 초기화) | `datasets` 테이블이 없으면 만든다(멱등). flow 시작 시 1회 |
| `register(...)` | ② 데이터 버전 관리 · ③ lineage | 데이터 업로드 후, 새 버전 레코드(경로·해시·`prefect_run_id`·metadata)를 장부에 추가 |
| `find(...)` | ① 검색·선택·다운로드 | `dataset_id`·`metadata` 키로 장부를 조회해 어떤 버전이 있는지 찾는다 |
| `list_datasets()` / `versions()` / `tree()` | ① 탐색(둘러보기) | 어떤 데이터셋·버전이 있는지 **목록/계층**으로 훑어본다 (아래 CLI) |

```
Prefect @task 안에서:
   ensure_schema()                         # 테이블 보장
   ...데이터 생성 → MinIO 업로드(boto3)...
   catalog.register(dataset_id, version, minio_path, prefect_run_id=..., metadata=...)   # 장부에 쓰기
다른 팀원 / 다음 단계에서:
   rows = catalog.find("sydney_202605", fab="fab2")    # 장부 조회 → 선택 → 다운로드
```
...
- `prefect_run_id` 로 "이 데이터를 만든 실행"을 연결해 계보(lineage)를 추적합니다([Chap. 9](#3-end-to-end-lineage-양방향-계보) 참고).

#### 스키마를 코드로 두기 — DB 생성과 테이블 생성을 분리

- **DB(`catalog`) 생성** → 인프라 1회: `docker-compose.yml` 의 init SQL 이 컨테이너 최초 기동 시 만듭니다([Appendix A](#a-docker-setup)).
- **테이블(`datasets`) 생성** → **코드로 관리**: `CREATE TABLE IF NOT EXISTS` 를 `catalog.py` 에 두고 **flow 시작 시 멱등(idempotent)하게 호출**합니다. 스키마가 코드와 함께 버전 관리되고 확장이 쉽습니다.

전체 구현은 **[`catalog.py`](catalog.py)** 를 참고합니다(연결 주소는 환경변수 `POSTGRESQL_CATALOG_DSN` 으로 덮어쓰기 가능). 핵심 API 요지는 다음과 같습니다.
...
# 데이터 업로드 후 새 버전 등록 (metadata 는 dict → JSONB)
catalog.register("sydney_202605", "v1", "s3://datasets/sydney_202605/v1/",
                 created_by="zoo", prefect_run_id="<run_id>",
                 metadata={"fab": "fab2", "chamber": "CH3", "step": 5})

rows = catalog.find("sydney_202605", fab="fab2")   # 검색 (dataset_id + metadata 키)
row  = catalog.get("sydney_202605")                # 최신 버전 한 행(없으면 None)
```

> Prefect flow 안에서 `ensure_schema()` → 데이터 업로드 후 `register(...)` 순으로 호출하면, 데이터 생성과 동시에 카탈로그가 채워집니다. 검색은 `find('sydney_202605', fab='fab2')` 처럼 `metadata` 키로 바로 거를 수 있습니다.

#### 카탈로그 둘러보기 — 이력을 모르는 팀원의 탐색·선택

처음 합류한 팀원은 "어떤 데이터가 있고 어떻게 골라야 하는지" 모릅니다. **전용 GUI는 없고**, 다음 세 가지로 탐색합니다.

1. **`catalog.py` CLI (권장 — 가장 간단, 추가 설치 불필요)** — `datasets` 장부를 목록/계층으로 보여줍니다.
   ```powershell
   python catalog.py list                        # 데이터셋 목록 (버전 수·최신 버전·최근 갱신)
   python catalog.py versions sydney_202605      # 한 데이터셋의 버전 이력 (최신→과거)
   python catalog.py tree                         # 데이터셋 > 버전 계층 트리
   python catalog.py find sydney_202605 fab=fab2  # metadata 키=값 으로 검색
   ```
   ```
   # 예: python catalog.py list
   dataset_id      versions  latest_version  latest_by  last_updated
   --------------  --------  --------------  ---------  -------------------
   sydney_202605   3         v3              zoo        2026-06-10 14:02:11
   melbourne_2026  1         v1              foo        2026-06-08 09:30:00

   # 예: python catalog.py tree
   sydney_202605
     └─ v1
     └─ v2
     └─ v3
   ```
   목록에서 원하는 `dataset_id`/`version`을 고른 뒤, 그 `minio_path`로 MinIO에서 내려받습니다([Chap. 9](#1-data-catalog--selective-retrieval--검색--선택--다운로드) 코드).

2. **범용 SQL 클라이언트** (DBeaver·pgAdmin·`psql`) — `catalog` DB의 `datasets` 테이블을 직접 조회합니다.
   ```sql
   SELECT dataset_id, version, created_by, created_at, minio_path
   FROM datasets ORDER BY created_at DESC;
   ```

3. **MinIO 콘솔(:9001)** — 버킷의 **실제 파일/폴더**를 봅니다. 단, "그 데이터가 무슨 의미인지(메타데이터)"는 보여주지 않으므로, 의미 탐색은 1·2번을 사용합니다.

> **"무엇이 있나"는 catalog(1·2번), "실제 파일 현황"은 MinIO 콘솔(3번)** 로 확인합니다. 필요 시 웹 카탈로그 UI(Streamlit 등)나 DataHub 같은 데이터 카탈로그 솔루션을 추가로 연동할 수 있습니다.

#### 추후 보강 (Extending the Schema)

데이터에 새 속성을 더 기록하고 싶을 때 두 가지 방법이 있습니다. **기준은 "그 속성이 모든 행에 항상 있고 자주 필터/조인하느냐"** 입니다.

1. **`metadata` JSONB 에 키 추가 (기본·권장 — 스키마 변경 불필요)**
   대부분의 보강은 여기서 끝납니다. 컬럼을 늘리지 않고 dict에 키만 추가하면 됩니다(임시·실험적·희소한 속성에 적합).
   ```python
   catalog.register("sydney_202605", "v3", "s3://datasets/sydney_202605/v3/",
                    metadata={"fab": "fab2", "chamber": "CH3", "recipe": "R12", "temp_c": 230})
   # 검색도 바로: catalog.find("sydney_202605", recipe="R12")
   ```
   자주 거르는 키는 표현식 인덱스로 빠르게:
   ```sql
   CREATE INDEX IF NOT EXISTS idx_datasets_fab ON datasets ((metadata->>'fab'));
   ```

2. **정식 컬럼으로 승격 (자주 쓰고 타입·제약이 중요할 때)**
   `ALTER TABLE` 로 컬럼을 추가하고, `catalog.py` 의 `DDL`/`register()` 에도 같은 컬럼을 반영합니다. 기존 행은 NULL로 채워지므로 안전합니다.
   ```sql
   ALTER TABLE datasets ADD COLUMN IF NOT EXISTS label_count INT;
   ```
   변경 이력을 코드로 관리하려면 **Alembic** 같은 마이그레이션 도구로 버전을 찍어두는 것을 권장합니다.

> 정리: **임시·희소 속성 → `metadata` JSONB**, **항상 존재하고 자주 필터하는 핵심 속성 → 정식 컬럼**.

### C. Attaching a Worker to a Work Pool from a Local PC (선택)
...
오케스트레이터 **[`example/prefect_run.py`](example/prefect_run.py)** 는 `example/` 단계 스크립트(`train_dp.py` … `test_eval.py`)를 `@task`로 감싸 순서대로 실행하고, **각 단계 산출물을 MinIO에 업로드 + [`catalog.py`](catalog.py)로 등록**합니다.

```
docker_prefect/
├─ catalog.py                  # 카탈로그 접근 계층 (Appendix B)
└─ example/
   ├─ prefect_run.py           # 오케스트레이터 (catalog/MinIO 배선 포함)
...
#### 현재 구현 — catalog + MinIO 배선

각 단계 task가 stage를 실행한 뒤 산출물을 **publish(업로드 + 등록)** 합니다.

```python
import catalog                       # 카탈로그 헬퍼 (Appendix B)

def _publish(stage, local_file, bucket, ctx, **extra):
    """단계 산출물 1개를 MinIO 업로드 + catalog 등록 (스택 없으면 경고만)."""
    key = f"{ctx['member']}/{ctx['experiment']}/{ctx['version']}/{os.path.basename(local_file)}"
    minio_path = f"s3://{bucket}/{key}"
    _upload(local_file, bucket, key)                       # boto3 → MinIO
    catalog.register(f"{ctx['member']}/{ctx['experiment']}/{stage}", ctx["version"], minio_path,
                     created_by=ctx["member"], prefect_run_id=ctx["run_id"],
                     metadata={"stage": stage, **extra})   # 메타데이터 → catalog DB

@task
def t_train_dp(ctx):
    out = train_dp.run()                                   # 단계 실행(로컬 산출물)
    _publish("train_dp", out, "datasets", ctx)             # MinIO + catalog 로 publish
    return out
...
- **graceful**: catalog/MinIO가 안 떠 있어도(스택 미기동) `register`/`upload`는 경고만 출력하고 **로컬 파이프라인은 끝까지 실행**됩니다.

> 각 단계 스크립트는 단독 실행도 가능합니다(`python example/train_dp.py`). `objective`(Optuna 목적 함수)의 의미는 `example/train.py`의 주석을 참고하세요.
>
> 참고: `example/`의 단계 스크립트 자체는 **흐름을 보여주는 최소 stub**입니다. MinIO 업로드·catalog 등록은 위처럼 **오케스트레이터(`prefect_run.py`)가 단계 바깥에서 감싸 처리**하고, **MLflow 로깅은 아직 미배선**입니다(MLflow까지 포함한 지향 모습은 아래 **완성형 패턴** 참고).

#### 완성형 패턴 — 모든 단계를 MinIO + MLflow + Optuna로 감싸기

[Chap. 7](#7-pipeline)의 원칙을 코드로 옮기면, **모든 단계가 동일하게 MinIO(download/upload) · MLflow(log) · Optuna(설정 탐색)로 감싸집니다.** 위 "현재 구현"에서 아직 빠진 **MLflow 로깅까지 포함한 지향 모습**입니다. (MinIO 초기 셋업은 [Chap. 3](#3-minio) 참고)
