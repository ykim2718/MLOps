...
POSTGRESQL="192.168.0.8:5432"                # PostgreSQL host:port (catalog / optuna DBs)
MLFLOW_TRACKING_URI="http://192.168.0.8:5000"   # MLflow tracking server

while [ $# -gt 0 ]; do
    case "$1" in
        --minio)      MINIO_ENDPOINT="$2"; shift 2 ;;
        --postgresql) POSTGRESQL="$2"; shift 2 ;;
...
set_var postgresql          "$POSTGRESQL"       # host:port; consumers (catalog.py / pipeline.py) split it
set_var mlflow_tracking_uri "$MLFLOW_TRACKING_URI"
echo "[register_variables] set: minio_endpoint, postgresql, mlflow_tracking_uri"
