...
> 아래 예시는 **`datasets` 버킷의 `SYDNEY/` 폴더에 `001.parquet` … `010.parquet`** 가 있다고 가정합니다(객체 키: `SYDNEY/001.parquet` … `SYDNEY/010.parquet`). 자격증명은 코드에 박지 말고 환경변수/파라미터로 주입하세요([§4](#4-read-only--scoped-access-keys)).
...
# ── list: SYDNEY/ 폴더의 객체 나열 (페이지네이션) ──
for page in s3.get_paginator("list_objects_v2").paginate(Bucket="datasets", Prefix="SYDNEY/"):
    for obj in page.get("Contents", []):
        print(obj["Key"], obj["Size"])               # SYDNEY/001.parquet 12345 ...

# ── download: 객체 1개 → 로컬 파일 ──
s3.download_file("datasets", "SYDNEY/001.parquet", "001.parquet")

# ── download: 폴더(SYDNEY/) 통째로 — prefix 의 모든 객체를 받아 같은 구조로 저장 ──
for page in s3.get_paginator("list_objects_v2").paginate(Bucket="datasets", Prefix="SYDNEY/"):
    for obj in page.get("Contents", []):
        key = obj["Key"]                                  # 예: SYDNEY/001.parquet
        os.makedirs(os.path.dirname(key), exist_ok=True)  # 로컬에 SYDNEY/ 폴더 생성
        s3.download_file("datasets", key, key)            # → ./SYDNEY/001.parquet … 010.parquet

# ── upload: 로컬 파일 → 객체 키 ──
s3.upload_file("001.parquet", "datasets", "SYDNEY/001.parquet")

# ── streaming: 다운로드(디스크 저장) 없이 메모리로 바로 읽기 ──
body = s3.get_object(Bucket="datasets", Key="SYDNEY/001.parquet")["Body"].read()
df = pd.read_parquet(io.BytesIO(body))               # 로컬 파일 안 만들고 DataFrame 으로

# ── 대용량: Range 로 일부 바이트만 부분 읽기 (전체를 안 받고 앞부분/특정 구간만) ──
head = s3.get_object(Bucket="datasets", Key="SYDNEY/001.parquet",
...
mc cat local/datasets/SYDNEY/001.parquet | python read_parquet.py    # 파이프로 바로 처리
```
...
s3 = boto3.client("s3", endpoint_url="http://localhost:9000",
                  aws_access_key_id="member1", aws_secret_access_key="<member1-secret>")
s3.download_file("datasets", "mnist-train.parquet", "mnist-train.parquet")  # OK (읽기)
# s3.upload_file(...)  → AccessDenied (쓰기 금지)
```

> ⚠️ **주의**: 읽기 전용 키로는 학습 파이프라인이 결과를 **업로드(`s3://models/...`)하거나 저장할 수 없습니다.** 따라서 **데이터를 내려받기만 하는 소비자형 팀원**에게 적합합니다(예: `data_downloader.ps1`). 팀원이 직접 파이프라인을 돌려 산출물을 남겨야 한다면 아래 (2) 처럼 **자기 영역에만 쓰기**를 허용하세요.
...
발급한 **Access Key / Secret Key** 는 코드에 박지 말고 **환경변수나 파라미터**로 주입합니다(`boto3`·`catalog.py`·`data_*.ps1` 가 자동으로 읽음).

```powershell
# 환경변수로 (boto3 / catalog.py / data_*.ps1 공통)
...
# 또는 스크립트 파라미터로
.\data_downloader.ps1 -DatasetId mnist -Endpoint http://<서버>:9000 -AccessKey <key> -SecretKey <secret>
```
