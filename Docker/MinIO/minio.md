...
MinIO 는 도커 컨테이너로 실행됩니다. 아래는 MinIO 의 `docker-compose.yml` 입니다. `docker compose up -d` 를 실행하면 도커가 `minio/minio` 이미지를 자동으로 내려받아 컨테이너로 띄우므로 **MinIO 를 호스트에 따로 설치할 필요가 없습니다.** `minio/minio` 이미지에는 `mc` 클라이언트가 함께 들어 있어, **한 서비스가 서버 기동과 버킷 생성·버저닝까지 모두 처리** 합니다.
...
services:
  minio:
    ...
    volumes:
      - minio-data:/data
    # 서버를 백그라운드로 띄우고 → 준비되면 mc 로 버킷 생성 + 버저닝 → wait 로 서버를 포그라운드 유지.
    entrypoint: >
      /bin/sh -c "
...
- `image: minio/minio` 는 공식 MinIO 서버 이미지를 사용한다는 뜻이며, 이 이미지에는 `mc` 클라이언트도 함께 들어 있어 같은 컨테이너 안에서 버킷을 만들 수 있습니다.
- `entrypoint` 는 서버를 **백그라운드로 띄운 뒤**(`minio server ... &`), `until` 로 서버가 준비될 때까지 재시도하여 alias 를 잡고, `mc` 로 `datasets`/`models`/`mlflow` 버킷을 만들고 `datasets`/`models` 에 버저닝을 켭니다. 마지막 `wait` 가 백그라운드 서버 프로세스를 기다려 **컨테이너를 계속 떠 있게** 합니다(이게 없으면 mc 명령 후 컨테이너가 종료됩니다).
  - 컨테이너 **안** 이라 엔드포인트는 서비스명이 아니라 `http://localhost:9000` 입니다.
  ...
  - 커뮤니티 웹 콘솔에는 버킷/버저닝 관리 메뉴가 없어 이렇게 `mc` 로 자동 처리하므로, 보통 `mc` 를 따로 설치하지 않아도 버킷·버저닝이 준비됩니다.
...
`mc`(MinIO Client)는 MinIO 서버와 **별개의 CLI 도구**입니다. 두 가지 방법으로 쓸 수 있습니다.
...
> 컨테이너 **안** 에서는 엔드포인트가 `http://localhost:9000`(같은 컨테이너의 서버)입니다. 호스트에 설치한 `mc` 로 접속할 때는 `http://<서버>:9000` 을 씁니다([§4](#4-read-only--scoped-access-keys) 의 alias 등록 참고).
...
docker 컨테이너 셸로 확인할 경우(호스트에 mc 를 설치하지 않아도 됨), 두 가지 방법이 있습니다.

**① 떠 있는 `minio` 컨테이너 셸에서(권장)** — `minio/minio` 이미지에 `mc` 가 있고 기동 시 `local` alias 가 이미 설정돼 있어, alias 등록 없이 바로 확인됩니다.
...
**② 별도 1회용 `minio/mc` 컨테이너로** — 같은 네트워크(`mlops`)에 붙여 서비스명(`minio:9000`)으로 접속하고 alias 를 직접 등록합니다.
