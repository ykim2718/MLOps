...
- 호스트 파이썬·`mc` 는 이 루트 계정을 access key/secret key 로 써서 접속합니다 (`MINIO_ROOT_USER`=access key, `MINIO_ROOT_PASSWORD`=secret key). 코드에 박지 말고 환경변수 (예: `MINIO_ACCESS_KEY` / `MINIO_SECRET_KEY`) 로 둡니다.
